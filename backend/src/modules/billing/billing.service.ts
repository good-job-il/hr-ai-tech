...
export class BillingService {
  constructor(
    ...
    @InjectRepository(BillingAccountEntity) private readonly accounts: Repository<BillingAccountEntity>,
    @InjectRepository(BillingInvoiceEntity) private readonly invoices: Repository<BillingInvoiceEntity>,
  ...
  async overview(user: UserEntity) {
    ...
    const [account, invoices, activeJobs, candidates, seats] = await Promise.all([
      ...
      this.invoices.find({ where: { organization_id: organization.id }, order: { issued_at: 'DESC' }, take: 50 }),
      this.jobs.count({ where: { organization_id: organization.id, state: 'open', is_deleted: false } }),
