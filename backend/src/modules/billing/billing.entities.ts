...
export class BillingAccountEntity extends BaseEntity {
  ...
  @Column({
    ...
    type: "enum",
    enum: ["trialing", "active", "past_due", "cancelled", "unpaid", "not_configured"],
  ...
  @Column({
    name: "payment_status",
    type: "enum",
    enum: ["paid", "pending", "failed", "not_configured"],
    default: "not_configured",
  })
...
export class BillingInvoiceEntity extends BaseEntity {
  ...
  @Column({ name: "invoice_number", type: "varchar", length: 100, nullable: true }) invoice_number:
    string | null
  ...
  @Column({ name: "hosted_invoice_url", type: "text", nullable: true }) hosted_invoice_url:
    string | null
