...
export class MessagesService {
  ...
  async findAll(query: QueryMessagesDto, user: UserEntity) {
    ...
    if (!application_id) return buildPaginatedResponse([], 0, { page, limit, sort, order });
  ...
  async create(dto: CreateMessageDto, user: UserEntity): Promise<MessageEntity> {
    await this.applications.findById(dto.application_id, user);
    // Prevent identity spoofing: sender identity is always derived from the authenticated user,
    // never trusted from the request body.
    const msg = this.repo.create({
      ...dto,
      sender_email: user.email,
      sender_role: user.role === 'candidate' ? 'candidate' : user.role === 'employer' ? 'employer' : 'recruiter',
      is_read: false,
    } as any);
  ...
  async markAllRead(applicationId: number, user: UserEntity): Promise<void> {
    ...
    const messages = await this.repo.find({ where: { application_id: applicationId, is_read: false } });
    const incoming = messages.filter(message => message.sender_email !== user.email).map(message => message.id);
    if (incoming.length) await this.repo.update(incoming, { is_read: true });
  }
}
