...
import {
  ...
  QueryAgencyClientsDto,
  UpdateAgencyClientDto,
} from "./dto/agency-clients.dto"
...
export class AgencyClientsService {
  constructor(
    ...
    private readonly clientRepo: Repository<AgencyClientEntity>,
  ...
  async findAll(query: QueryAgencyClientsDto, user: UserEntity) {
    ...
    if (query.search) {
      qb.andWhere(
        "(company.name LIKE :search OR client.contact_name LIKE :search OR client.contact_email LIKE :search)",
        {
          search: `%${query.search}%`,
        },
      )
  ...
  async create(dto: CreateAgencyClientDto, user: UserEntity) {
    ...
    const createdId = await this.dataSource.transaction(async (manager) => {
      ...
      if (dto.company_id) {
        company = await manager.findOne(CompanyEntity, {
          where: { id: dto.company_id, is_deleted: false },
        })
      ...
      if (duplicate) {
        throw new ConflictException("Company is already a client of this agency")
      }
  ...
  async update(id: number, dto: UpdateAgencyClientDto, user: UserEntity) {
    ...
    if (client.status === "archived") {
      throw new ConflictException("Archived client cannot be edited")
    }

    const company = await this.companyRepo.findOne({
      where: { id: client.company_id, is_deleted: false },
    })

    if (!company) {
      throw new NotFoundException(`Company ${client.company_id} not found`)
    }
  ...
  private async hydrate(clients: AgencyClientEntity[], organizationId: number) {
    ...
    const companies = await this.companyRepo.find({
      where: { id: In(companyIds), is_deleted: false },
    })
    ...
    const appRows = await this.applicationRepo
      ...
      .addSelect(
        "SUM(CASE WHEN application.status IN ('new','reviewed','phone_interview','recommended','employer_interview','offer','probation') THEN 1 ELSE 0 END)",
        "in_process",
      )
      .addSelect(
        "SUM(CASE WHEN application.status IN ('hired','completed') THEN 1 ELSE 0 END)",
        "hired",
      )
  ...
  private async assertAccountManager(
    ...
    if (!manager) {
      throw new ForbiddenException("Invalid account_manager_id assignment")
    }
  }
}
