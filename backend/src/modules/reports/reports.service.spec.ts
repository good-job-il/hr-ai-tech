...
describe('ReportsService OA-3', () => {
  it('builds metrics only from the current organization scope', async () => {
    ...
    applications.find.mockResolvedValue([
      ...
      { id: 2, organization_id: 12, status: 'probation', source: 'pool_assignment', recruiter_id: 7, team_manager_id: 8, employer_company_id: 9, job_id: 10, company: 'Client', job_title: 'Engineer', created_date: created, updated_date: new Date('2026-01-11T00:00:00Z') },
    ]);
    ...
    const permissions = { getEffectivePermissions: jest.fn().mockResolvedValue({ permissions: { view_compensation: true } }) };
    const service = new ReportsService(applications as any, timelines as any, repo() as any, repo() as any, users as any, permissions as any);
    ...
    expect(result.summary).toEqual(expect.objectContaining({ applications: 2, placements: 1, average_time_to_hire_days: 5 }));
    expect(result.source_effectiveness[0]).toEqual(expect.objectContaining({ source: 'pool_assignment', conversion_rate: 50 }));
  });
});
