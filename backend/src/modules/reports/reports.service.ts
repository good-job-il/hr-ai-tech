...
export class ReportsService {
  constructor(
    ...
    @InjectRepository(ApplicationTimelineEntity) private readonly timelines: Repository<ApplicationTimelineEntity>,
  ...
  async getManagementReport(query: ManagementReportQueryDto, user: UserEntity) {
    ...
    if (query.client_id) where.employer_company_id = query.client_id;
    ...
    transitionEvents.filter(event => event.new_value === 'hired').forEach(event => {
      if (!hiredAt.has(event.application_id)) hiredAt.set(event.application_id, event.created_date);
    });
    ...
    const source = this.group(apps, app => app.source || 'unknown', value => ({
      source: value,
      applications: 0,
      placements: 0,
      conversion_rate: 0,
    ...
    const recruiter = this.performance(apps, app => app.recruiter_id, id => userNames.get(id) || `#${id}`, 'recruiter_id', 'recruiter_name');
    const team = this.performance(apps, app => app.team_manager_id, id => teamNames.get(id) || userNames.get(id) || `#${id}`, 'team_manager_id', 'team_name');
    const clients = this.performance(apps, app => app.employer_company_id, id => apps.find(app => app.employer_company_id === id)?.company || `#${id}`, 'client_id', 'client_name');
    const jobs = this.performance(apps, app => app.job_id, id => apps.find(app => app.job_id === id)?.job_title || `#${id}`, 'job_id', 'job_title');
    ...
    for (const app of apps) {
      ...
      if (!['completed', 'rejected'].includes(stage)) {
        const duration = Math.max(0, Math.min(Date.now(), to.getTime()) - enteredAt.getTime()) / 86400000;
        stageDurations.set(stage, [...(stageDurations.get(stage) || []), duration]);
      }
    }
    ...
    placements.forEach(app => {
      const plan = plans.find(item => item.job_id === app.job_id)
      ...
      allocatedCompensation += this.amount(plan.recruiter_compensation, plan.recruiter_compensation_type, fee);
      allocatedCompensation += this.amount(plan.team_manager_compensation, plan.team_manager_compensation_type, fee);
      allocatedCompensation += this.amount(plan.recruitment_manager_compensation, plan.recruitment_manager_compensation_type, fee);
    });
    ...
    return {
      ...
      job_conversion: jobs,
      dimensions: {
        recruiters: members.filter(member => member.role === UserRole.RECRUITER).map(member => ({ id: member.id, name: userNames.get(member.id) })),
