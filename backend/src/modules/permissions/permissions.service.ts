...
export class PermissionsService {
  ...
  async findMatrices(query: QueryPermissionMatricesDto, user: UserEntity) {
    ...
    if (this.isPrivileged(user) && org_type) {
      ;(where as Record<string, any>).org_type = org_type
    }

    if (role_key) {
      if (Array.isArray(where)) {
        where = where.map((item) => ({ ...item, role_key }))
      } else {
  ...
  async updateMatrix(
    ...
    if (!matrix) {
      throw new NotFoundException(`Permission matrix ${id} not found`)
    }
  ...
  async updateRoleTemplate(
    ...
    if (!this.isPrivileged(user)) {
      ...
      if (dto.display_name !== undefined) {
        tpl.display_name = dto.display_name
      }
  ...
  async updateUserPositionAccess(
    ...
    if (!access) {
      throw new NotFoundException(`User position access ${id} not found`)
    }
