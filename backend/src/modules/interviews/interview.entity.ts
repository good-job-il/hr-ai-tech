...
export class InterviewEntity extends BaseEntity {
  ...
  @Column({ type: 'enum', enum: ['phone', 'video', 'in_person', 'technical', 'hr', 'final'], default: 'video' })
  type: string;

  @Column({ type: 'enum', enum: ['screening', 'first', 'second', 'third', 'technical', 'hr', 'final', 'offer'], default: 'first', nullable: true })
  stage: string | null;
  ...
  @Column({ type: 'enum', enum: ['scheduled', 'confirmed', 'completed', 'cancelled', 'no_show', 'rescheduled'], default: 'scheduled' })
  status: string;
