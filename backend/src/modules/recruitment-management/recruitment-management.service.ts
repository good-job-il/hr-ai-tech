...
export class RecruitmentManagementService {
  ...
  async dashboard(user: UserEntity, sendAlerts = false, query: ManagementReportQueryDto = {}) {
    ...
    const [organizationApplications, organizationJobs, recruiters, teams] = await Promise.all([
      ...
      this.users.find({ where: { organization_id: organizationId, role: UserRole.RECRUITER, is_active: true } }),
    ...
    if (query.team_id && !selectedTeam) throw new NotFoundException(`Team ${query.team_id} not found`);
    const applications = organizationApplications.filter(application =>
    ...
    const jobs = organizationJobs.filter(job =>
    ...
    const overdue = applications
      .filter(application => ACTIVE_APPLICATION_STATUSES.has(application.status) && APPLICATION_SLA_HOURS[application.status])
      .map(application => ({
        id: application.id,
        ...
        overdue_hours: Math.max(0, Math.floor((now - new Date(application.updated_date).getTime()) / 3_600_000) - APPLICATION_SLA_HOURS[application.status]),
      }))
      .filter(application => application.overdue_hours > 0)
      .sort((a, b) => b.overdue_hours - a.overdue_hours);

    const teamByManager = new Map(teams.filter(team => team.manager_id).map(team => [team.manager_id, team]));
    const workload = recruiters.map(recruiter => {
      const activeApplications = applications.filter(application =>
        application.recruiter_id === recruiter.id && ACTIVE_APPLICATION_STATUSES.has(application.status)).length;
      const openJobs = jobs.filter(job => job.recruiter_id === recruiter.id && !job.is_closed).length;
      return {
    ...
    const funnel = Object.fromEntries(APPLICATION_PIPELINE_STATUSES
      .map(status => [status, applications.filter(application => application.status === status).length]));
    const placements = applications.filter(application => PLACEMENT_APPLICATION_STATUSES.has(application.status));
    const dashboard = {
      summary: {
        open_jobs: jobs.filter(job => !job.is_closed).length,
        applications: applications.length,
        active_applications: applications.filter(application => ACTIVE_APPLICATION_STATUSES.has(application.status)).length,
      ...
      dimensions: {
        recruiters: recruiters.map(recruiter => ({ id: recruiter.id, name: recruiter.full_name || recruiter.email })),
        teams: teams.map(team => ({ id: team.id, name: team.name })),
        clients: [...new Map(organizationApplications
          .filter(application => application.employer_company_id)
          .map(application => [application.employer_company_id, {
  ...
  async assign(dto: AssignRecruitmentWorkDto, user: UserEntity) {
    ...
    const result = await this.dataSource.transaction(async manager => {
      ...
      if (dto.team_id != null) {
        team = await manager.findOne(AgencyTeamEntity, { where: { id: dto.team_id, organization_id: organizationId, is_active: true } });
      ...
      if (dto.recruiter_id != null) {
        recruiter = await manager.findOne(UserEntity, {
          where: { id: dto.recruiter_id, organization_id: organizationId, role: UserRole.RECRUITER, is_active: true },
        });
        if (!recruiter) throw new NotFoundException(`Recruiter ${dto.recruiter_id} not found`);
        if (team && recruiter.team_id !== team.id) throw new BadRequestException('Recruiter must belong to the selected team');
        if (!team && recruiter.team_id) {
          team = await manager.findOne(AgencyTeamEntity, { where: { id: recruiter.team_id, organization_id: organizationId, is_active: true } });
        }
      }

      const [jobs, candidates, applications] = await Promise.all([
        jobIds.length ? manager.find(JobEntity, { where: { id: In(jobIds), organization_id: organizationId, is_deleted: false } }) : [],
        candidateIds.length ? manager.find(CandidateEntity, { where: { id: In(candidateIds), organization_id: organizationId, is_deleted: false } }) : [],
        applicationIds.length ? manager.find(ApplicationEntity, { where: { id: In(applicationIds), organization_id: organizationId, is_deleted: false } }) : [],
      ]);
      if (jobs.length !== jobIds.length) throw new NotFoundException('One or more jobs were not found');
      if (candidates.length !== candidateIds.length) throw new NotFoundException('One or more candidates were not found');
      if (applications.length !== applicationIds.length) throw new NotFoundException('One or more applications were not found');
      ...
      applications.forEach(record => Object.assign(record, target, { assigned_to: recruiter?.id ?? null }));
      ...
      if (applications.length) {
        await manager.save(ApplicationTimelineEntity, applications.map(application => manager.create(ApplicationTimelineEntity, {
          application_id: application.id,
      ...
      await manager.save(AuditLogEntity, manager.create(AuditLogEntity, {
        ...
        action: 'update',
        metadata: {
          operation: 'bulk_reassign', reason: dto.reason,
          job_ids: jobIds, candidate_ids: candidateIds, application_ids: applicationIds,
          team_id: team?.id ?? null, recruiter_id: recruiter?.id ?? null,
        },
      }));
    ...
    if (result.recruiter_id) {
      const recruiter = await this.users.findOne({ where: { id: result.recruiter_id, organization_id: organizationId } });
      if (recruiter?.email) {
  ...
  async refreshOperationalAlerts() {
    const managers = await this.users.find({
      where: { role: UserRole.RECRUITMENT_MANAGER, org_type: OrgType.STAFFING_AGENCY, is_active: true },
    });
  ...
  private async sendOperationalAlerts(user: UserEntity, dashboard: Awaited<ReturnType<RecruitmentManagementService['dashboard']>>) {
    ...
    if (dashboard.summary.overloaded_recruiters > 0) {
      await this.notifications.createUnreadOnce({
        ...
        metadata: { alert_type: 'workload_overload', count: dashboard.summary.overloaded_recruiters },
      } as any);
    }
  }
}
