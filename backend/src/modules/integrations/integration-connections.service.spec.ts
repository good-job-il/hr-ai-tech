...
describe("IntegrationConnectionsService OA-3 feature flags", () => {
  it("does not expose a connect action when a provider is disabled", async () => {
    ...
    expect(
      catalog.every((item) => item.feature_available === false && item.status === "disconnected"),
    ).toBe(true)
    await expect(service.connect("gmail", {} as any, actor)).rejects.toBeInstanceOf(
      ConflictException,
    )
  ...
  it("persists connect and disconnect state when the provider is supported", async () => {
    ...
    const config = {
      get: jest.fn((key, fallback) => {
        if (key === "INTEGRATION_GMAIL_ENABLED") {
          return "true"
        }

        if (key === "INTEGRATION_GMAIL_CONNECT_URL") {
  ...
  it("rejects recruitment manager integration mutations before persistence", async () => {
    ...
    await expect(service.connect("gmail", {} as any, manager)).rejects.toBeInstanceOf(
      ForbiddenException,
    )
