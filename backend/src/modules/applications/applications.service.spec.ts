...
describe('ApplicationsService OA-2 lifecycle', () => {
  ...
  it('prevents reopening a completed application', async () => {
    ...
    await expect(service.update(1, { status: 'reviewed' } as any, actor))
  ...
  it('blocks reopening through the generic status endpoint', async () => {
    ...
    await expect(service.changeStatus(2, 'reviewed' as any, undefined, actor))
      .rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('reopens a rejected application through the explicit audited operation', async () => {
    ...
    await expect(service.reopen(2, 'reviewed' as any, 'Candidate requested reconsideration', actor))
  ...
  it('rejects a duplicate candidate and job pair before insert', async () => {
    jobs.findOne.mockResolvedValue({ id: 9, organization_id: 11, employer_company_id: 3, title: 'Engineer' });
