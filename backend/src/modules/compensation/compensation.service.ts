...
export class CompensationService {
  constructor(
    ...
    private readonly users: Repository<OrganizationUserEntity>,
...
return buildPaginatedResponse([], 0, { page, limit })
  ...
  private async resolveRelations(dto: Record<string, any>, user: UserEntity) {
    ...
    if (!user.organization_id && !isPlatformAdmin) {
    ...
    const job = dto.job_id
      ? await this.jobs.findOne({
          where: isPlatformAdmin
            ? { id: dto.job_id }
            : { id: dto.job_id, organization_id: organizationId },
        })
      : null
    ...
    if (!companyId) {
      throw new ForbiddenException("An agency client or job is required")
    }

    if (job?.employer_company_id !== companyId) {
      throw new ForbiddenException("Job and client do not belong together")
    }
    ...
    if (!client && !isPlatformAdmin) {
      throw new ForbiddenException("The selected company is not an active agency client")
    }
    ...
    const assigneeIds = [dto.recruiter_id, dto.team_manager_id, dto.recruitment_manager_id].filter(
...
throw new ForbiddenException("Job and assignees must belong to one team")
  ...
  private async logChange(
    ...
    await this.audit.log({
      ...
      entity_type: "CompensationPlan",
      entity_id: plan.id,
      entity_label: plan.client_name,
      action,
