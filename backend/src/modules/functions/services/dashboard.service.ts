...
export class DashboardService {
  ...
  async getDashboardStats() {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      trendData.push({
        day: label,
        candidates: recentCandidates.filter(
          (c) => c.created_date >= dayStart && c.created_date < dayEnd,
        ).length,
        jobs: recentJobs.filter((j) => j.created_date >= dayStart && j.created_date < dayEnd)
          .length,
        applications: recentApplications.filter(
          (a) => a.created_date >= dayStart && a.created_date < dayEnd,
        ).length,
        imports: batches.filter((b) => b.created_date >= dayStart && b.created_date < dayEnd)
          .length,
      })
    }
