...
import { GetJobRecommendationsDto, SmartSearchDto } from "../dto/functions.dto"
...
export class MatchingService {
  ...
  private calculateJobSimilarity(job1: JobEntity, job2: JobEntity): number {
    ...
    const hoursSinceCreation =
      (Date.now() - new Date(job2.created_date).getTime()) / (1000 * 60 * 60)
  ...
  async getRecommendedJobs(user: UserEntity) {
    ...
    const savedJobIds = (await this.savedJobRepo.find({ where: { user_id: user.id } })).map(
      (s) => s.job_id,
    )

    const appliedJobIds = (await this.appRepo.find({ where: { candidate_user_id: user.id } })).map(
      (a) => a.job_id,
    )
  ...
  private heuristicCandidateJobScore(profile: CandidateProfileEntity, job: JobEntity): number {
    ...
    const jobSkills = [...(job.required_skills || []), ...(job.preferred_skills || [])].map((s) =>
      s.toLowerCase(),
    )
    ...
    if (profile.job_type && job.type && profile.job_type === job.type) {
      score += 10
    }
  ...
  async scoreApplication(applicationId: number, user: UserEntity) {
    ...
    await this.applicationsService.update(
      app.id,
      { match_score: score, match_reason: reason } as any,
      user,
    )
  ...
  async smartSearch(dto: SmartSearchDto) {
    ...
    if (type === "autocomplete") {
      ...
      jobs.forEach((job) => {
        ...
        if (job.company?.toLowerCase().startsWith(queryLower)) {
          suggestions.add(job.company)
        }

        if (job.category?.toLowerCase().includes(queryLower)) {
          suggestions.add(job.category)
        }
      })
      roles.forEach((role) => {
        if (role.name?.toLowerCase().startsWith(queryLower)) {
    ...
    let allJobs = await this.jobRepo.find({
      where: { is_closed: false, is_deleted: false },
      order: { created_date: "DESC" },
      take: 500,
    })
    ...
    if (filters.type?.length) {
      filtered = filtered.filter((x) => filters.type!.includes(x.job.type))
    }

    if (filters.location) {
      filtered = filtered.filter((x) =>
        x.job.location?.toLowerCase().includes(filters.location!.toLowerCase()),
      )
    }

    if (filters.category?.length) {
      filtered = filtered.filter((x) => filters.category!.includes(x.job.category ?? ""))
    }
