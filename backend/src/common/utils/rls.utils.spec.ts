...
describe("getRlsWhere", () => {
  it("prevents an organization admin from overriding tenant scope", () => {
    expect(
      getRlsWhere("Candidate", user({ role: UserRole.ORG_ADMIN, organization_id: 10 }), {
        organization_id: 99,
      }),
  ...
  it("confines recruiters to their tenant and own assignment", () => {
    expect(
      getRlsWhere("Application", user({ role: UserRole.RECRUITER, organization_id: 10 }), {
        organization_id: 99,
        recruiter_id: 123,
      }),
  ...
  it("confines candidates to canonical user ownership", () => {
    expect(getRlsWhere("Application", user({ id: 55 }), { candidate_user_id: 99 })).toEqual({
      is_deleted: false,
      candidate_user_id: 55,
    })
  })

  it("blocks unsupported employer resources", () => {
    const filter = getRlsWhere(
      "AuditLog",
      user({
