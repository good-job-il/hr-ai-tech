...
function scoreDomain(candidate, job) {
  if (!candidate.domain_id || !job.domain_id) {
    return 50
  }
...
function scoreRole(candidate, job) {
  ...
  if (jTitle.includes(cRole) || cRole.includes(jTitle)) {
    return 85
  }
...
function scoreLocation(candidate, job) {
  ...
  if (jLoc.includes("remote") || jLoc.includes("מרחוק") || job.type === "remote") {
    return 90
  }

  if (cLoc.includes(jLoc) || jLoc.includes(cLoc)) {
    return 80
  }
...
function buildExplanation(candidate, job, breakdown, score) {
  ...
  if (breakdown.location >= 80) {
    strengths.push(t("aiMatching.engine.strengths.locationMatch"))
  }

  if (breakdown.salary >= 70) {
    strengths.push(t("aiMatching.engine.strengths.salaryInRange"))
  }
  ...
  if (breakdown.role < 50) {
    gaps.push(t("aiMatching.engine.gaps.roleMismatch"))
  }

  if (breakdown.skills < 40) {
    gaps.push(t("aiMatching.engine.gaps.skillsGap"))
  }
  ...
  if (breakdown.salary < 40) {
    gaps.push(t("aiMatching.engine.gaps.salaryOutOfRange"))
  }

  if (breakdown.location < 40) {
    gaps.push(t("aiMatching.engine.gaps.locationMismatch"))
  }
  ...
  if (breakdown.salary < 30) {
    risks.push(t("aiMatching.engine.risks.salaryGap"))
  }

  if (score < 50) {
    risks.push(t("aiMatching.engine.risks.lowScore"))
  }
