...
export default function Register() {
  ...
  const USER_TYPES = [
    {
      id: "candidate",
      label: isRtl ? "מועמד (מחפש עבודה)" : "Candidate (Job Seeker)",
      requiresOrg: false,
    },
    { id: "employer", label: isRtl ? "מעסיק (חברה)" : "Employer (Company)", requiresOrg: true },
    {
      id: "org_admin",
      label: isRtl ? "בעל חברה / אדמין" : "Company Owner / Admin",
      requiresOrg: true,
    },
  ]
  ...
  const ORG_TYPES = [
    {
      ...
      desc: isRtl
        ? "גיוס עצמאי ללא מערכת תגמולים פנימית"
        : "Independent hiring without internal compensation system",
      icon: "🏢",
    },
    {
      id: "staffing_agency",
      ...
      desc: isRtl
        ? "כולל מערכת תגמולים, עמלות וניהול היררכי"
        : "Includes compensation, commissions and hierarchical management",
      icon: "🎯",
    },
  ]
      ...
      <div className="w-full max-w-md">
        <div ref={cardRef} className="bg-white rounded-2xl shadow-xl p-8 md:p-10">
          ...
          <>
            <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
                  ...
                  <p className="text-xs text-blue-700">
                    {isRtl
                      ...
                      : "Your organization and role are securely defined by the invitation"}
                  </p>
                </div>
              )}
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full bg-red-600 hover:bg-red-700 text-white h-12 font-bold text-base rounded-lg"
              >
                {loading ? (
          ...
          <p className="text-center text-sm text-gray-600 mt-7">
            ...
            <Link to="/login" className="text-hhblue font-bold hover:underline">
        ...
        <div className="mt-8 text-center space-y-2 text-xs text-gray-500">
          ...
          <p>
            ✓{" "}
            {isRtl
              ? "בנוי עבור מחפשי עבודה ומעסיקים בישראל"
              : "Built for job seekers and employers in Israel"}
          </p>
        </div>
      </div>
    </div>
