...
export default function AgencyTeamsPage() {
  ...
  const { user } = useAuth(); const { can } = usePermissionMatrix();
  ...
  return <PlatformPageShell dir={isRtl ? 'rtl' : 'ltr'}><div className="mx-auto max-w-7xl space-y-6">
    <PlatformPageHeader title={t('agencyTeams.title')} subtitle={t('agencyTeams.subtitle')} icon={Users} actions={canManageMembers && <Button onClick={() => setInviteOpen(true)}><Plus className="me-2 h-4 w-4" />{t('agencyTeams.inviteMember')}</Button>} />
