...
export default function AgencyDashboard() {
  ...
  const orgName =
    organization?.name || user?.organization_name || t("agencyDashboard.defaultOrganization")
    ...
    <PlatformPageShell dir={isRtl ? "rtl" : "ltr"}>
      <div className="space-y-7">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <PlatformStatCard
            icon={Briefcase}
            label={t("agencyDashboard.stats.openJobs.label")}
          ...
          <PlatformStatCard
            ...
            meta={t("agencyDashboard.stats.compensationPlans.meta")}
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <PlatformCard className="p-6">
            <PlatformWidgetHeader
              title={t("agencyDashboard.recentJobs.title")}
              linkTo="/agency/jobs/open"
            ...
            {loading ? (
              ...
              <PlatformEmptyState icon={Briefcase}>
                {t("agencyDashboard.recentJobs.empty")}
              </PlatformEmptyState>
            ) : (
              <div className="space-y-2">
                {recentJobs.map((job) => (
                  <div
                    key={job.id}
                    className="flex items-center justify-between rounded-2xl border border-slate-100/80 bg-slate-50/65 px-4 py-3 transition hover:border-[#DDD6FE] hover:bg-[#F8F5FF]"
                  >
                    <div>
          ...
          <PlatformCard className="p-6">
            <PlatformWidgetHeader
              title={t("agencyDashboard.quickActions.title")}
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                {
                  icon: Briefcase,
                  label: t("agencyDashboard.quickActions.postJob"),
                  to: "/agency/jobs/open",
                  color: "text-[#7C3AED]",
                  bg: "bg-[#F3EFFF]",
                },
                {
                  icon: Users,
                  label: t("agencyDashboard.quickActions.addCandidate"),
                  to: "/agency/crm",
                  color: "text-blue-600",
                  bg: "bg-blue-50",
                },
                {
                  icon: Kanban,
                  label: t("agencyDashboard.quickActions.viewPipeline"),
                  to: "/agency/pipeline",
                ...
                {
                  icon: Sparkles,
                  label: t("agencyDashboard.quickActions.aiMatching"),
                ...
                {
                  icon: DollarSign,
                  label: t("agencyDashboard.quickActions.manageCompensation"),
                  to: "/agency/compensation",
                ...
                {
                  icon: Activity,
                  label: t("agencyDashboard.quickActions.activityLog"),
                  to: "/agency/activity",
                ...
                >
                  <span
                    className={`flex h-9 w-9 items-center justify-center rounded-xl ${a.bg} ${a.color}`}
                  >
