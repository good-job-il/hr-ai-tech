    ...
    <div className="min-h-screen flex items-center justify-center px-4" dir={isRtl ? 'rtl' : 'ltr'} style={{ backgroundColor: '#eaf7fb' }}>
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        <div className="text-center mb-6">
          <div className="w-14 h-14 bg-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-lg mx-auto mb-3">HH</div>
        ...
        {sent ? (
          <div className="text-center">
            ...
            <p className="text-green-700 font-semibold mb-1">{isRtl ? 'שלחנו לך אימייל!' : 'Email sent!'}</p>
            ...
            <Link to="/login" className="text-blue-600 text-sm font-semibold hover:underline">{t('auth.forgotPassword.backToLogin')}</Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center">
              <Link to="/login" className="text-blue-600 text-sm hover:underline">{isRtl ? '← ' : ''}{t('auth.forgotPassword.backToLogin')}</Link>
            </p>
          </form>
        )}
      </div>
