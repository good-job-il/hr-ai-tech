    ...
    <div className="bg-white rounded-2xl border border-[#E4ECFF] p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
      ...
      <div className="flex-1 min-w-0">
        {loading ? (
          ...
          <>
            <div className="text-2xl font-black text-[#0F172A]">{value?.toLocaleString() ?? '—'}</div>
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {change !== undefined && (
                <span className={`flex items-center gap-0.5 text-xs font-bold ${isPositive ? 'text-emerald-600' : 'text-red-500'}`}>
                  ...
                  {Math.abs(change)}%
                  <span className="text-[#94A3B8] font-normal">משבוע</span>
                </span>
              )}
            </div>
          </>
...
const ICON_MAP = {
  UserPlus, FileText, Activity, CheckCircle2, XCircle, Clock, Download, Zap, Shield, Database,
  Calendar: Clock, MessageSquare: FileText, Send: ArrowUpRight,
};
  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[#64748B] mt-0.5">
            ...
            {lastUpdated && <span className="mr-3 text-xs text-[#94A3B8]">עודכן: {lastUpdated.toLocaleTimeString('he-IL')}</span>}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
        <StatCard icon={FileText} label="ייבואים (סה״כ אצוות)" value={stats?.batchCount} color="#F59E0B" loading={loading} />
        <StatCard icon={Briefcase} label="משרות פתוחות" value={stats?.openJobs} color="#3B82F6" loading={loading} />
        <StatCard icon={Users} label="מועמדים במערכת" value={stats?.candidates} color="#8B5CF6" loading={loading} />
        <StatCard icon={Building2} label="חברות רשומות" value={stats?.companies} color="#6366F1" loading={loading} />
      </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E4ECFF] p-6">
          ...
          <div className="flex flex-col items-center">
            <ResponsiveContainer width="100%" height={160}>
              <RePieChart>
                <Pie data={importSourceData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value">
            ...
            <div className="w-full space-y-1.5 mt-2">
              {importSourceData.map((item, i) => (
                <div key={item.name} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full" style={{ background: IMPORT_SOURCE_COLORS[i] }} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-[#E4ECFF] p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/import-dashboard" className="text-xs font-bold text-[#7C3AED] hover:underline">הצג הכל</Link>
          </div>
          <div className="space-y-3">
            {recentImports.length === 0 && !loading ? (
              <div className="text-center py-6 text-[#94A3B8] text-sm font-semibold">אין ייבואים אחרונים</div>
              ...
              <div key={i} className="flex items-start gap-3">
                ...
                <span className="text-xs font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-lg flex-shrink-0">{imp.count} רשומות</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E4ECFF] p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/analytics/behavior" className="text-xs font-bold text-[#7C3AED] hover:underline">הצג הכל</Link>
          </div>
          <div className="space-y-3">
            {recentActivity.length === 0 && !loading ? (
              <div className="text-center py-6 text-[#94A3B8] text-sm font-semibold">אין פעילות אחרונה</div>
              ...
              return (
                <div key={i} className="flex items-start gap-3">
                  ...
                  <span className="text-[10px] text-[#94A3B8] flex-shrink-0 mt-0.5">{item.time}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E4ECFF] p-6">
          ...
          <div>
            ...
            <HealthRow label="ספק נתונים AI" status={stats?.pendingBatches > 0 ? 'warning' : 'ok'} loading={loading} />
            <HealthRow label="אדמין" status="active" loading={loading} />
            <HealthRow label="מעסיקים" status={stats?.openJobs > 0 ? 'active' : 'warning'} loading={loading} />
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-[#F1F5F9] space-y-2">
            ...
            {(stats?.reviewRequired ?? 0) > 0 && (
              <Link to="/admin/crm" className="block p-2.5 rounded-xl bg-amber-50 border border-amber-200 hover:bg-amber-100 transition-colors">
                <div className="text-xs font-black text-amber-700">{stats.reviewRequired} מועמדים דורשים בדיקה</div>
                <div className="text-[10px] text-amber-600">לחץ לצפייה ב-CRM</div>
              </Link>
            )}
            {(stats?.openJobs ?? 0) === 0 && (
              <Link to="/admin/jobs" className="block p-2.5 rounded-xl bg-red-50 border border-red-200 hover:bg-red-100 transition-colors">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-3">
        {[
          { icon: AlertCircle, label: 'כשלי ייבוא (סה״כ)', value: stats?.failedImports != null ? String(stats.failedImports) : '—', color: '#F59E0B' },
          { icon: FileText, label: 'רשומות יובאו בהצלחה', value: stats?.totalImported != null ? String(stats.totalImported) : '—', color: '#6366F1' },
          { icon: CheckCircle2, label: 'הגשות חדשות', value: stats?.newApplications != null ? String(stats.newApplications) : '—', color: '#10B981' },
          { icon: Activity, label: 'ראיונות מתוזמנים', value: stats?.scheduledInterviews != null ? String(stats.scheduledInterviews) : '—', color: '#3B82F6' },
          { icon: Clock, label: 'אצוות בתהליך', value: stats?.pendingBatches != null ? String(stats.pendingBatches) : '—', color: '#0891B2' },
        ].map((item) => {
          const Icon = item.icon;
          return (
            <div key={item.label} className="bg-white rounded-xl border border-[#E4ECFF] p-3 flex flex-col items-center text-center gap-1">
              ...
              <div className="text-[10px] text-[#94A3B8] font-semibold leading-tight">{item.label}</div>
            </div>
          );
        })}
      </div>
