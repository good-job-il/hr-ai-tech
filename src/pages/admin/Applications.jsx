...
export default function AdminApplications() {
  ...
  const filtered = applications.filter((app) => {
    ...
    return matchSearch && matchStatus && matchSource && matchEmployer && matchDateFrom && matchDateTo;
  });
    ...
    <AdminLayout>
      <div className="p-6" dir="rtl">
        ...
        <div className="flex gap-4 mb-5 flex-wrap">
          ...
          <div className="bg-white rounded-xl border border-gray-100 px-4 py-3 shadow-sm text-center min-w-[100px]">
            <div className="text-xl font-bold text-blue-600">{applications.filter(a => a.status === 'new').length}</div>
            <div className="text-xs text-gray-500">חדשות</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 px-4 py-3 shadow-sm text-center min-w-[100px]">
            <div className="text-xl font-bold text-emerald-600">{applications.filter(a => a.status === 'hired').length}</div>
            <div className="text-xs text-gray-500">התקבלו</div>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 px-4 py-3 shadow-sm text-center min-w-[100px]">
            <div className="text-xl font-bold text-red-500">{applications.filter(a => a.status === 'rejected').length}</div>
        ...
        <div className="flex gap-3 mb-4 flex-wrap">
          ...
          <select value={filterSource} onChange={e => setFilterSource(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none">
            <option value="">כל המקורות</option>
            {Object.entries(sourceLabels).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
          </select>
          <select value={filterEmployer} onChange={e => setFilterEmployer(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none max-w-[180px]">
            <option value="">כל המעסיקים</option>
            {employers.map(e => <option key={e} value={e}>{e}</option>)}
          </select>
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="מתאריך" />
          <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none" placeholder="עד תאריך" />
        </div>
                ...
                {filtered.map((app) => (
                  <tr key={app.id} className="bg-white hover:bg-gray-50 transition-colors">
                    <td className="px-3 py-3">
                      ...
                      {app.candidate_phone && <div className="text-xs text-gray-400">{app.candidate_phone}</div>}
                    </td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3 text-xs text-gray-600">
                      {app.desired_salary_min || app.desired_salary_max ? (
                        <span>₪{app.desired_salary_min?.toLocaleString() || '?'} – ₪{app.desired_salary_max?.toLocaleString() || '?'}</span>
                      ) : '—'}
                    </td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3 text-xs text-gray-500">{sourceLabels[app.source] || app.source || '—'}</td>
                    <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">
                      {new Date(app.created_date).toLocaleDateString('he-IL')}
                    </td>
                    <td className="px-3 py-3">
                      {app.resume_url ? (
                        <a href={app.resume_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-hhblue hover:underline text-xs">
