    ...
    <tr className="border-b border-slate-100 transition-colors hover:bg-violet-50/30">
      ...
      <td className="px-4 py-3 text-center">
        <span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${levelColor}`}>
          {t(`roleSettings.levels.${record.hierarchy_level}`, {
            defaultValue: String(record.hierarchy_level),
          })}
        </span>
      </td>
      ...
      <td className="px-4 py-3 text-center">
        <span
          className={`inline-block h-2.5 w-2.5 rounded-full ${record.is_active ? "bg-emerald-500 shadow-[0_0_0_3px_rgba(16,185,129,0.12)]" : "bg-slate-300"}`}
        />
      </td>
    </tr>
  )
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <PlatformStatCard
            ...
            value={editableRoles}
            tone="blue"
            meta={t("roleSettings.footer")}
          />
        </div>

        {/* Info banner */}
        <PlatformCard
          className="border-blue-100 bg-gradient-to-r from-blue-50/90 to-violet-50/70 px-5 py-4 text-sm leading-6 text-blue-700"
          dangerouslySetInnerHTML={{ __html: t("roleSettings.infoBanner") }}
        />

        {loading ? (
          <PlatformCard className="p-16 text-center text-slate-400">
            {t("roleSettings.loading")}
          </PlatformCard>
        ) : error ? (
        ...
        <p className="text-center text-xs text-slate-400">{t("roleSettings.footer")}</p>
      </div>
    </PlatformPageShell>
  )
}
