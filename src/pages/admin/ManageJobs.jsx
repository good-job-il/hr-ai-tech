      ...
      <div className="p-6" dir="rtl">
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {filtered.map((job) => (
              <div key={job.id} className={`bg-white rounded-xl border shadow-sm p-3 flex items-center justify-between ${job.is_closed ? 'opacity-60 border-gray-100' : 'border-gray-100'}`}>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-2 flex-wrap">
                      ...
                      {job.employer_id && SOURCE_LABELS[job.employer_id] && (
                        <span className="bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded text-xs">{SOURCE_LABELS[job.employer_id]}</span>
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${job.is_closed ? 'bg-gray-100 text-gray-500' : 'bg-green-100 text-green-700'}`}>
                  ...
                  >
                    {job.is_closed ? <ToggleLeft className="w-4 h-4" /> : <ToggleRight className="w-4 h-4" />}
                  </button>
                  <button onClick={() => deleteMutation.mutate(job.id)} className="p-2 hover:bg-red-50 rounded-lg text-red-400 transition-all active:scale-90">
