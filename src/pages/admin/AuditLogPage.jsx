/**
 * AuditLogPage — צפייה וניהול יומן אודיט
 * פילטרים: תאריך, משתמש, פעולה, ישות
 * יצוא CSV
 */
...
const ACTION_CONFIG = {
  view: { label: 'צפייה', icon: Eye, color: '#64748B' },
  create: { label: 'יצירה', icon: CheckCircle2, color: '#10B981' },
  update: { label: 'עדכון', icon: Activity, color: '#3B82F6' },
  delete: { label: 'מחיקה', icon: XCircle, color: '#EF4444' },
  cv_download: { label: 'הורדת CV', icon: Download, color: '#8B5CF6' },
  cv_view: { label: 'צפייה ב-CV', icon: Eye, color: '#6366F1' },
  status_change: { label: 'שינוי סטטוס', icon: Activity, color: '#F59E0B' },
  send_to_employer: { label: 'שליחה למעסיק', icon: FileText, color: '#06B6D4' },
  export: { label: 'יצוא', icon: Download, color: '#14B8A6' },
  compensation_change: { label: 'שינוי תגמול', icon: Activity, color: '#EC4899' },
  login: { label: 'כניסה', icon: CheckCircle2, color: '#10B981' },
  impersonate: { label: 'התחזות', icon: AlertTriangle, color: '#F97316' },
  restore: { label: 'שחזור', icon: CheckCircle2, color: '#10B981' },
};
...
export default function AuditLogPage() {
  ...
  const exportToCSV = () => {
    const headers = ['תאריך', 'משתמש', 'פעולה', 'ישות', 'מזהה ישות', 'תיאור', 'IP'];
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-[#64748B] mt-0.5">ניטור פעילויות משתמשים ושינויים במערכת</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E4ECFF] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-[#0F172A]">פילטרים</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
              <label className="text-xs font-semibold text-[#64748B] mb-1 block">ישות</label>
            ...
            <div>
              <label className="text-xs font-semibold text-[#64748B] mb-1 block">פעולה</label>
            ...
            <div>
              <label className="text-xs font-semibold text-[#64748B] mb-1 block">אימייל משתמש</label>
              <input type="text" value={filters.actor_email} onChange={e => updateFilter('actor_email', e.target.value)}
                placeholder="חיפוש..." className="w-full h-9 px-3 bg-[#F7FBFF] border border-[#E4ECFF] rounded-xl text-sm focus:outline-none focus:border-[#7C3AED]" />
            </div>
            <div>
              <label className="text-xs font-semibold text-[#64748B] mb-1 block">מתאריך</label>
              <input type="date" value={filters.date_from} onChange={e => updateFilter('date_from', e.target.value)}
                className="w-full h-9 px-3 bg-[#F7FBFF] border border-[#E4ECFF] rounded-xl text-sm focus:outline-none focus:border-[#7C3AED]" />
            </div>
            <div>
              <label className="text-xs font-semibold text-[#64748B] mb-1 block">עד תאריך</label>
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="flex items-center gap-2">
              <Button size="sm" onClick={clearFilters} variant="outline" className="text-xs gap-1">
                <XCircle className="w-3.5 h-3.5" /> נקה פילטרים
              </Button>
              <span className="text-xs text-[#94A3B8]">{logs.length} רשומות בעמוד {page + 1}</span>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[#F7FBFF] border-b border-[#E4ECFF]">
                <tr>
                  <th className="text-right text-xs font-bold text-[#64748B] p-3">תאריך</th>
                  <th className="text-right text-xs font-bold text-[#64748B] p-3">משתמש</th>
                  <th className="text-right text-xs font-bold text-[#64748B] p-3">פעולה</th>
                  <th className="text-right text-xs font-bold text-[#64748B] p-3">ישות</th>
                  <th className="text-right text-xs font-bold text-[#64748B] p-3">תיאור</th>
                  <th className="text-left text-xs font-bold text-[#64748B] p-3"></th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                          ...
                          <tr>
                            <td colSpan={6} className="bg-[#F7FBFF] p-4 border-t border-[#E4ECFF]">
                              <div className="grid grid-cols-2 gap-3 text-xs">
                                <div>
                                  <span className="font-semibold text-[#64748B]">מזהה ישות:</span>
                                  <span className="text-[#374151] mr-2 font-mono">{log.entity_id}</span>
                                </div>
                                <div>
                                  <span className="font-semibold text-[#64748B]">תפקיד מבצע:</span>
                                  <span className="text-[#374151] mr-2">{log.actor_role || '—'}</span>
                                </div>
                                {log.ip_address && (
                                  <div>
                                    <span className="font-semibold text-[#64748B]">IP:</span>
                                    <span className="text-[#374151] mr-2 font-mono">{log.ip_address}</span>
                                  </div>
                                )}
                                {log.user_agent && (
                                  <div className="col-span-2">
                                    <span className="font-semibold text-[#64748B]">User-Agent:</span>
                                    <span className="text-[#374151] mr-2 block mt-1 font-mono text-[10px]">{log.user_agent}</span>
                                  </div>
                                )}
                                {log.metadata && (
                                  <div className="col-span-2">
