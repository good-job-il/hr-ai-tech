...
const CATEGORIES = ['פיתוח תוכנה', 'עיצוב', 'שיווק', 'מכירות', 'כספים', 'HR', 'הנדסה', 'רפואה', 'חינוך', 'לוגיסטיקה', 'אחר'];
...
export default function CandidateProfilePage() {
  ...
  React.useEffect(() => {
    if (profile && !form) {
      ...
      setForm({
        ...
        experience: [], desired_salary_min: '', desired_salary_max: '',
        job_type: 'any', categories: [], is_public: true, resume_url: '',
      });
    }
  }, [profile, user]);
  ...
  const addExp = () => setForm(f => ({ ...f, experience: [...(f.experience || []), { company: '', role: '', years: '', description: '' }] }));
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[['full_name','שם מלא'], ['phone','טלפון'], ['location','מיקום'], ['title','תפקיד מבוקש']].map(([key, label]) => (
                <div key={key}>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={form.summary || ''} onChange={e => setForm(f => ({ ...f, summary: e.target.value }))}
                  rows={3} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-hhblue/30 resize-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200">
            ...
            <div className="flex gap-2 mb-3">
              ...
              <button onClick={addSkill} className="bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-700 hover:to-violet-700 text-white px-3.5 h-10 rounded-xl text-sm font-medium shadow-md hover:shadow-lg transition-all active:scale-95">
                <Plus className="w-4 h-4" />
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {(form.skills || []).map((s, i) => (
                <span key={i} className="flex items-center gap-1 bg-hhblue/10 text-hhblue text-xs px-3 py-1.5 rounded-full">
                  {s}
                  <button onClick={() => removeSkill(i)} className="hover:text-red-500"><Trash2 className="w-3 h-3" /></button>
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200">
            <div className="flex justify-between items-center mb-3">
              ...
              <button onClick={addExp} className="text-purple-600 text-sm font-medium hover:text-purple-700 transition-colors flex items-center gap-1"><Plus className="w-3.5 h-3.5" />הוסף</button>
            </div>
            <div className="space-y-3">
              {(form.experience || []).map((exp, i) => (
                <div key={i} className="border border-gray-100 rounded-xl p-3 space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <input value={exp.company} onChange={e => updateExp(i, 'company', e.target.value)} placeholder="חברה"
                      className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none" />
                    <input value={exp.role} onChange={e => updateExp(i, 'role', e.target.value)} placeholder="תפקיד"
                      className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none" />
                    <input value={exp.years} onChange={e => updateExp(i, 'years', e.target.value)} placeholder="שנים (למשל: 2020–2023)"
                      className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none col-span-2" />
                  </div>
                  <textarea value={exp.description} onChange={e => updateExp(i, 'description', e.target.value)} placeholder="תיאור קצר"
                    rows={2} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none resize-none" />
                  <button onClick={() => removeExp(i)} className="text-red-500 text-xs font-medium hover:text-red-700 transition-colors flex items-center gap-1 active:scale-95">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input type="number" value={form.desired_salary_min || ''} onChange={e => setForm(f => ({ ...f, desired_salary_min: +e.target.value }))}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none" />
              </div>
              <div>
            ...
            <label className="flex items-center gap-2 mt-2 text-sm cursor-pointer">
              <input type="checkbox" checked={form.is_open_to_work || false} onChange={e => setForm(f => ({ ...f, is_open_to_work: e.target.checked }))} />
              <span className={`font-medium ${form.is_open_to_work ? 'text-green-600' : 'text-gray-700'}`}>
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200">
            ...
            <label className="flex items-center gap-2 bg-gray-50 border border-dashed border-gray-300 rounded-xl p-4 cursor-pointer hover:bg-gray-100 transition-colors">
              ...
              <span className="text-sm text-gray-600">{form.resume_url ? 'עדכן קובץ' : 'העלה קורות חיים'}</span>
              <input type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={uploadResume} />
            </label>
            {form.resume_url && (
              <a href={form.resume_url} target="_blank" rel="noreferrer" className="text-hhblue text-sm mt-2 hover:underline block">
                צפה בקובץ שהועלה
              </a>
            )}
          </div>
