...
const CATEGORIES = ['', 'פיתוח תוכנה', 'עיצוב', 'שיווק', 'מכירות', 'כספים', 'HR', 'הנדסה', 'רפואה', 'חינוך'];
const typeLabels = { any: 'כל הסוגים', full: 'משרה מלאה', part: 'חלקית', remote: 'מרחוק', daily: 'יומי' };

export default function JobAlerts() {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ keywords: '', location: '', category: '', job_type: 'any', salary_min: '', frequency: 'daily' });
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#eaf7fb' }} dir="rtl">
      ...
      <div className="max-w-[700px] mx-auto px-4 py-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => setShowForm(true)} className="bg-hhblue text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-hhblue/90 flex items-center gap-2">
        ...
        {showForm && (
          <div className="bg-white rounded-2xl p-5 border border-gray-100 mb-4 shadow-sm">
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowForm(false)} className="flex-1 border border-gray-200 text-gray-600 py-2 rounded-lg text-sm hover:bg-gray-50">ביטול</button>
        ...
        {alerts.length === 0 ? (
          ...
          <div className="space-y-3">
            {alerts.map(alert => (
              <div key={alert.id} className={`bg-white rounded-xl border p-4 flex items-center justify-between ${!alert.is_active ? 'opacity-60' : 'border-gray-100'}`}>
                <div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleMutation.mutate(alert)} className="text-gray-400 hover:text-hhblue p-1">
                    {alert.is_active ? <Bell className="w-4 h-4" /> : <BellOff className="w-4 h-4" />}
                  </button>
                  <button onClick={() => deleteMutation.mutate(alert.id)} className="text-gray-400 hover:text-red-500 p-1">
