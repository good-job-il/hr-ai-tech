...
import {
  ...
  getNotifColor,
  NOTIF_TYPE_LABELS,
} from "@/components/notifications/notifHelpers"
    ...
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        {loadError && (
          <div
            role="status"
            className="mb-4 rounded-lg border border-amber-400/30 bg-amber-500/10 p-3 text-sm text-amber-300"
          >
            {loadError}
          </div>
        )}
        ...
        <div className="flex gap-2 mb-4">
          {[
            ...
            { key: "unread", label: `לא נקרא (${unreadCount})` },
            { key: "read", label: "נקרא" },
          ].map((f) => (
            <button
              key={f.key}
          ...
          {loading ? (
            ...
            filtered.map((notif) => (
              ...
              >
                {/* Icon */}
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${getNotifColor(notif.type)}`}
                >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span
                          className={`text-sm font-semibold ${notif.is_read ? "text-gray-300" : "text-white"}`}
                        >
                          {notif.title}
                        </span>
