  ...
  return (
    <button
      ...
      className={`w-full text-start px-4 py-3 border-b border-[#F0F1F5] transition-colors last:border-0 ${
        isSelected ? "bg-[#F3EFFF]" : "hover:bg-[#F7F8FC]"
      }`}
    >
...
function ChatWindow({ application, user, onClose }) {
  ...
  const { data: messages = [], isLoading } = useQuery({
    ...
    queryFn: () => messageService.list({ application_id: application.id, limit: 100 }),
    enabled: !!application,
  ...
  const sendMutation = useMutation({
    mutationFn: () => messageService.send(application.id, text),
    onSuccess: () => {
      setText("")
            ...
            return (
              <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                ...
                >
                  ...
                  <div
                    className={`text-xs mt-1 ${isMine ? "opacity-70 text-white" : "text-[#94A3B8]"}`}
                  >
...
function SelectConversationPrompt() {
  ...
  return (
    <div className="flex-1 bg-white rounded-2xl border border-[#E4ECFF] flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm font-semibold text-[#64748B] mt-1">
        {t("candidate.messages.selectHint")}
      </p>
    </div>
  )
}
...
export default function CandidateMessages() {
  ...
  const {
    ...
    queryFn: () => applicationService.list({ sort: "created_date", order: "DESC", limit: 50 }),
    enabled: !!user?.email,
  })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 min-h-[520px]">
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-12 px-4 text-center">
                ...
                <p className="font-bold text-[#94A3B8] text-sm">
                  {t("candidate.messages.noConversations")}
                </p>

                <p className="text-xs text-[#CBD5E1] mt-1">
                  {t("candidate.messages.noConversationsHint")}
                </p>
              </div>
            ) : (
              applications.map((app) => (
        ...
        {/* Chat or empty state */}
        {selected ? (
          <ChatWindow application={selected} user={user} onClose={() => setSelected(null)} />
        ) : (
