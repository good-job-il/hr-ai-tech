...
export default function CandidateProfile() {
  ...
  const saveBtnLabel =
    saveStatus === "saving"
      ? t("candidate.profile.saving")
      : saveStatus === "saved"
        ? t("candidate.profile.saved")
        : saveStatus === "error"
          ? t("candidate.profile.saveError")
  ...
  return (
    <div dir={isRtl ? "rtl" : "ltr"} className="space-y-5 max-w-3xl">
      ...
      <SectionCard
        icon={GraduationCap}
        title={`${t("candidate.profile.education")} & ${t("candidate.profile.preferences")}`}
        accent="#EA580C"
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
      ...
      <SectionCard icon={FileText} title={t("candidate.profile.resumeSection")} accent="#7C3AED">
        <label
          className="flex items-center gap-3 bg-[#F8F6FF] border-2 border-dashed border-[#C4B5FD] rounded-xl p-5
