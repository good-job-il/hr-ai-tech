  ...
  return (
    <div dir="rtl" className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={load} disabled={loading} className="h-9 w-9 rounded-xl border border-[#E4ECFF] flex items-center justify-center text-[#64748B] hover:border-[#C4B5FD] disabled:opacity-50">
          <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
        </button>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <StatCard icon={Zap} label="הגשות שלי" value={stats?.applications ?? '—'} color="#2563EB" loading={loading} />
        <StatCard icon={Target} label="ראיונות קרובים" value={stats?.interviews ?? '—'} color="#059669" loading={loading} />
        <StatCard icon={BookOpen} label="משרות שמורות" value={stats?.savedJobs ?? '—'} color="#EA580C" loading={loading} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#E4ECFF] p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/candidate/jobs/all" className="text-sm font-bold text-[#7C3AED] hover:underline">כל המשרות</Link>
          </div>
          {loading ? (
            ...
            <div className="space-y-3">
              {recentJobs.map(job => (
                <Link key={job.id} to={`/jobs/${job.id}`} className="flex items-center gap-3 p-3 rounded-xl border border-[#F0F1F5] hover:border-[#C4B5FD] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-[#94A3B8] truncate">{job.company} • {job.location}</div>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E4ECFF] p-6">
          ...
          <div className="space-y-2">
            <Link to="/candidate/jobs/all" className="flex items-center gap-2 p-3 rounded-xl border border-[#E4ECFF] hover:border-[#C4B5FD] transition-colors text-sm font-bold text-[#374151]">
              <Sparkles className="w-4 h-4 text-[#7C3AED]" /> חפש משרות
            </Link>
            <Link to="/candidate/profile" className="flex items-center gap-2 p-3 rounded-xl border border-[#E4ECFF] hover:border-[#C4B5FD] transition-colors text-sm font-bold text-[#374151]">
