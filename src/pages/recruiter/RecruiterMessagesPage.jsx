...
export default function RecruiterMessagesPage() {
  ...
  const CHANNEL_LABELS = { email: 'אימייל', whatsapp: 'WhatsApp', phone: 'טלפון', sms: 'SMS', in_app: 'באפליקציה', other: 'אחר' };
  const DIR_COLORS = { outbound: 'bg-blue-100 text-blue-700', inbound: 'bg-green-100 text-green-700' };
    ...
    <div dir="rtl" className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-[#E4ECFF] overflow-hidden">
          {logs.map((log, i) => (
            <div key={log.id} className={`flex items-start gap-4 px-5 py-4 ${i < logs.length - 1 ? 'border-b border-[#F0F1F5]' : ''} hover:bg-[#F7F8FC] transition-colors`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-sm font-bold text-[#0F172A] truncate">{log.candidate_email}</span>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${DIR_COLORS[log.direction] || 'bg-gray-100 text-gray-600'}`}>
                    {log.direction === 'outbound' ? 'יוצא' : 'נכנס'}
                  </span>
                  <span className="text-xs text-[#94A3B8]">{CHANNEL_LABELS[log.channel] || log.channel}</span>
                </div>
                {log.subject && <div className="text-xs font-semibold text-[#64748B] truncate">{log.subject}</div>}
