    ...
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map(i => (
        <button key={i} type="button" onClick={() => onChange && onChange(i)}>
          <Star className={`w-5 h-5 ${i <= value ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'}`} />
        </button>
      ))}
    </div>
  );
...
export default function CompanyProfile() {
  ...
  const avgRating = reviews.length ? (reviews.reduce((s, r) => s + r.rating_overall, 0) / reviews.length).toFixed(1) : null;
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#eaf7fb' }} dir="rtl">
      ...
      <div className="max-w-[900px] mx-auto px-4 py-6">
        <Link to="/companies" className="text-hhblue hover:underline text-sm flex items-center gap-1 mb-5">
        ...
        {jobs.length > 0 && (
          <div className="bg-white rounded-2xl p-5 border border-gray-100 mb-4">
            ...
            <div className="space-y-2">
              {jobs.map(j => (
                <Link key={j.id} to={`/jobs/${j.id}`} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                  <div>
                    <div className="text-sm font-medium text-gray-900 group-hover:text-hhblue">{j.title}</div>
                    <div className="text-xs text-gray-400 mt-0.5">{j.location}</div>
                  </div>
                  {j.salary_min && <div className="text-xs text-green-600 font-medium">₪{j.salary_min.toLocaleString()}+</div>}
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            ...
            {user && !showReviewForm && (
              <button onClick={() => setShowReviewForm(true)} className="flex items-center gap-1 text-hhblue text-sm hover:underline">
          ...
          {showReviewForm && (
            <form onSubmit={e => { e.preventDefault(); submitReview.mutate(reviewForm); }}
              ...
              <div>
                ...
                <StarRating value={reviewForm.rating_overall} onChange={v => setReviewForm(f => ({ ...f, rating_overall: v }))} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[['rating_salary', 'שכר ותגמולים'], ['rating_management', 'ניהול'], ['rating_worklife', 'איזון עבודה-חיים']].map(([key, label]) => (
                  <div key={key}>
                    <label className="text-xs text-gray-500 block mb-1">{label}</label>
                    <StarRating value={reviewForm[key]} onChange={v => setReviewForm(f => ({ ...f, [key]: v }))} />
                  </div>
                ))}
              </div>
              ...
              <textarea value={reviewForm.pros} onChange={e => setReviewForm(f => ({ ...f, pros: e.target.value }))}
                placeholder="יתרונות" rows={2}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none resize-none" />
              <textarea value={reviewForm.cons} onChange={e => setReviewForm(f => ({ ...f, cons: e.target.value }))}
                placeholder="חסרונות" rows={2}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none resize-none" />
              <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                <input type="checkbox" checked={reviewForm.is_anonymous} onChange={e => setReviewForm(f => ({ ...f, is_anonymous: e.target.checked }))} />
                פרסם בעילום שם
              </label>
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowReviewForm(false)} className="flex-1 border border-gray-200 py-2 rounded-lg text-sm">ביטול</button>
                <button type="submit" disabled={!reviewForm.rating_overall || submitReview.isPending}
          ...
          {reviews.length === 0 ? (
            <div className="text-center py-8 text-gray-400 text-sm">אין ביקורות עדיין. היה הראשון לכתוב!</div>
          ) : (
