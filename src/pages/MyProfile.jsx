...
export default function MyProfile() {
  ...
  const { data: saved = [] } = useQuery({
    ...
    queryFn: () =>
      httpClient.get(
        `/jobs/saved?user_email=${encodeURIComponent(user.email)}&sort=created_date&order=DESC&limit=50`,
        { cache: false },
      ),
    enabled: !!user,
  })
  ...
  const { data: applications = [] } = useQuery({
    ...
    queryFn: () =>
      httpClient.get(
        `/applications?candidate_email=${encodeURIComponent(user.email)}&sort=created_date&order=DESC&limit=50`,
        { cache: false },
      ),
    enabled: !!user,
  })
  ...
  const { data: interviews = [] } = useQuery({
    ...
    queryFn: () =>
      httpClient.get(
        `/interviews?candidate_email=${encodeURIComponent(user.email)}&sort=created_date&order=DESC&limit=50`,
        { cache: false },
      ),
    enabled: !!user,
  })
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-100">
                        {interviews
                          ...
                          .map((interview) => (
                            <div
                              key={interview.id}
                              className="bg-purple-50 rounded-lg p-3 text-sm text-purple-700 font-medium cursor-pointer hover:bg-purple-100 transition-colors"
                              onClick={() => setSelectedInterview(interview)}
                            >
                              🎉 זומנת לראיון -{" "}
                              {new Date(interview.date).toLocaleDateString("he-IL")} בשעה{" "}
                              {interview.time}
                            </div>
                          ))}
                      </div>
              ...
              {selectedInterview.location_or_link && (
                <div>
                  ...
                  {selectedInterview.type === "in_person" ? (
                    ...
                    <a
                      href={selectedInterview.location_or_link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-sm text-hhblue hover:underline break-all"
                    >
                      {selectedInterview.location_or_link}
                    </a>
                  )}
