...
function UserModal({ open, onClose, user, orgs, onSave, isSaving, t, isRTL }) {
  ...
  return (
    <PlatformModal
      dir={isRTL ? 'rtl' : 'ltr'}
      title={isEdit ? t('platform.usersManagement.modal.editTitle') : t('platform.usersManagement.modal.createTitle')}
    ...
    >
      <div className="max-h-[68vh] space-y-4 overflow-y-auto pe-1">
          {/* Full Name */}
          <div>
            <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.usersManagement.modal.fullName')} *</label>
            <input
          ...
          {/* Email */}
          <div>
            <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.usersManagement.modal.email')} *</label>
            <input
              type="email"
              value={form.email}
          ...
          {!isEdit && (
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.usersManagement.modal.password')} *</label>
              <input
                type="password"
                value={form.password}
              ...
              />
              <p className={`mt-1 text-xs ${errors.password ? 'font-semibold text-red-500' : 'text-slate-400'}`}>
          ...
          {/* Phone */}
          <div>
            <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.usersManagement.modal.phone')}</label>
            <input type="tel" value={form.phone} onChange={e => set('phone', e.target.value)} className={platformFieldClassName} />
          </div>

          {/* Role */}
          <div>
            <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.usersManagement.modal.role')}</label>
          ...
          {/* Organization */}
          <div>
            <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.usersManagement.modal.organization')}</label>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/60 p-3">
            <label className="text-sm font-bold text-slate-700">{t('platform.usersManagement.modal.status')}</label>
            <button type="button" onClick={() => set('is_active', !form.is_active)}
              ...
              <span className={`inline-block h-[18px] w-[18px] transform rounded-full bg-white shadow-sm transition-transform ${form.is_active ? (isRTL ? '-translate-x-1' : 'translate-x-[20px]') : (isRTL ? '-translate-x-[20px]' : 'translate-x-0.5')}`} />
            </button>
            <span className="text-sm font-medium text-slate-500">{form.is_active ? t('platform.usersManagement.modal.active') : t('platform.usersManagement.modal.inactive')}</span>
          </div>

        {/* Footer */}
        <div className={`flex gap-3 border-t border-slate-100 pt-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <Button variant="secondary" size="sm" className="flex-1" onClick={onClose} disabled={isSaving}>
  ...
  return (
    ...
    >
        ...
        <div className={`flex gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
          <Button variant="danger" size="sm" className="flex-1" onClick={onConfirm} disabled={isDeleting}>
            {isDeleting ? t('platform.usersManagement.deleteConfirm.deleting') : t('platform.usersManagement.deleteConfirm.confirm')}
          </Button>
          <Button variant="secondary" size="sm" className="flex-1" onClick={onClose} disabled={isDeleting}>
...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[60] flex items-center gap-2 rounded-2xl border border-white/20 px-5 py-3 text-sm font-bold text-white shadow-[0_16px_35px_rgba(30,41,59,0.2)] transition-all ${type === 'error' ? 'bg-rose-600' : 'bg-emerald-600'}`}>
      {message}
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="p-5">
                      <PlatformEmptyState icon={Search}>{t('platform.usersManagement.noUsers')}</PlatformEmptyState>
                    </td>
                  </tr>
                    ...
                    <tr key={u.id} className="group transition-colors hover:bg-violet-50/35">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`relative flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br text-xs font-black ${
                            index % 3 === 0
                            ...
                            <span className={`absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white ${u.is_active === false ? 'bg-slate-300' : 'bg-emerald-500'}`} />
                          </div>
                          <div>
                            <p className="font-extrabold text-slate-800 transition group-hover:text-violet-700">{u.full_name || '—'}</p>
                            <p className="mt-0.5 text-[10px] font-medium text-slate-400">{u.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <span className={`inline-flex rounded-full px-3 py-1 text-[10px] font-extrabold ${role.bg} ${role.text}`}>
                          {roleLabel}
                        </span>
                      </td>
                      <td className="px-5 py-4 text-xs font-semibold text-slate-600">{orgName}</td>
