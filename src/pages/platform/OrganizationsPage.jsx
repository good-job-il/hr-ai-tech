            ...
            <table className="w-full min-w-[920px] text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/60">
                  <th className="px-5 py-3.5 text-start text-[11px] font-extrabold uppercase tracking-[0.08em] text-slate-400">{t('platform.orgs.colName')}</th>
                  <th className="px-5 py-3.5 text-start text-[11px] font-extrabold uppercase tracking-[0.08em] text-slate-400">{t('platform.orgs.colType')}</th>
                  <th className="px-5 py-3.5 text-start text-[11px] font-extrabold uppercase tracking-[0.08em] text-slate-400">{t('platform.orgs.colEmail')}</th>
                  <th className="px-5 py-3.5 text-start text-[11px] font-extrabold uppercase tracking-[0.08em] text-slate-400">{t('platform.orgs.plan')}</th>
                  <th className="px-5 py-3.5 text-start text-[11px] font-extrabold uppercase tracking-[0.08em] text-slate-400">{t('platform.orgs.status')}</th>
                  <th className="px-5 py-3.5 text-start text-[11px] font-extrabold uppercase tracking-[0.08em] text-slate-400">{t('platform.orgs.actions')}</th>
                </tr>
              </thead>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="p-5">
                      <PlatformEmptyState icon={Search}>{t('platform.orgs.noResults')}</PlatformEmptyState>
                    </td>
                  </tr>
                  ...
                  return (
                    <tr key={org.id} className="group transition-colors hover:bg-violet-50/35">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-0.5 text-[10px] font-medium text-slate-400">ID: {String(org.id).slice(0, 8)}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4 text-xs font-medium text-slate-500">{org.contact_email || '—'}</td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="min-w-[170px] rounded-xl border-slate-100 p-1.5 shadow-xl">
                            <DropdownMenuItem
      ...
      {showModal && (
        ...
        >
          <div className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.orgs.labelOrgName')}</label>
              <input
                type="text"
                value={newOrg.name}
            ...
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.orgs.labelType')}</label>
            ...
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.orgs.labelEmail')}</label>
              <input
                type="email"
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="secondary" size="sm" className="flex-1" onClick={() => setShowModal(false)} disabled={creating}>
                {t('platform.orgs.btnCancel')}
              </Button>
              <Button variant="primary" size="sm" className="flex-1" onClick={handleCreate} disabled={creating || !newOrg.name.trim()}>
      ...
      {editOrg && (
        ...
        >
          <div className="space-y-4">
            ...
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.orgs.plan')}</label>
              <select
                value={editOrg.plan}
            ...
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-700">{t('platform.orgs.status')}</label>
      ...
      {deleteOrg && (
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <Button variant="danger" size="sm" className="flex-1" onClick={handleDelete} disabled={deleting}>
