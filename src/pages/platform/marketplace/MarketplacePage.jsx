...
export default function MarketplacePage() {
  ...
  const now = new Date();
  const active   = accesses.filter(a => a.is_active && (!a.expires_at || new Date(a.expires_at) > now));
  const expired  = accesses.filter(a => !a.is_active || (a.expires_at && new Date(a.expires_at) <= now));
  ...
  const exposedOrgIds = new Set(accesses.filter(a => a.access_type !== 'owner').map(a => a.owner_organization_id));
  const exposedOrgs = orgs.filter(o => exposedOrgIds.has(o.id));

  // Orgs that purchased / received access
  const buyerOrgIds = new Set(accesses.filter(a => a.accessor_organization_id).map(a => a.accessor_organization_id));
  ...
  const orgExposure = orgs.map(org => {
    const orgAccesses = accesses.filter(a => a.owner_organization_id === org.id && a.access_type !== 'owner');
    return { ...org, sharedCount: orgAccesses.filter(a => a.access_type === 'shared').length, purchasedCount: orgAccesses.filter(a => a.access_type === 'purchased').length, total: orgAccesses.length };
    ...
    <div dir="ltr" className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-semibold">Cross-organization candidate access and pool management</p>
        </div>
        <div className="flex gap-3">
          <Link to="/platform/marketplace/candidates"
          ...
          <Link to="/platform/marketplace/exposure"
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <KpiCard icon={ShoppingCart} label="Total Access Grants" value={accesses.filter(a => a.access_type !== 'owner').length} color="purple" loading={loading} to="/platform/marketplace/candidates" />
        <KpiCard icon={CheckCircle} label="Active Grants" value={active.filter(a => a.access_type !== 'owner').length} color="green" loading={loading} />
        <KpiCard icon={Share2} label="Shared Accesses" value={shared.length} color="blue" loading={loading} />
        <KpiCard icon={CreditCard} label="Purchased Accesses" value={purchased.length} color="amber" loading={loading} />
        <KpiCard icon={Building2} label="Orgs Exposing Pool" value={exposedOrgs.length} color="purple" loading={loading} to="/platform/marketplace/exposure" />
      </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                ...
                <span className="text-2xl font-black text-gray-600">{expired.filter(a => a.access_type !== 'owner').length}</span>
              </div>
            </div>
          )}
          <Link to="/platform/marketplace/candidates" className="flex items-center gap-1 mt-4 text-sm font-bold text-purple-600 hover:text-purple-700">
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 truncate">
                        {acc.granted_at ? new Date(acc.granted_at).toLocaleDateString('en-GB') : '—'}
                      </p>
                    </div>
                  </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link to="/platform/marketplace/exposure" className="flex items-center gap-1 text-sm font-bold text-purple-600 hover:text-purple-700">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {orgExposure.map(org => (
              <div key={org.id} className="p-4 border border-gray-100 rounded-xl hover:border-purple-200 transition-colors">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Active Grants', value: active.filter(a => a.access_type !== 'owner').length, sub: 'live now', color: 'green' },
            ...
            <div key={item.label} className={`p-4 rounded-xl ${
              ...
              item.color === 'green'  ? 'bg-emerald-50' : 'bg-amber-50'
            }`}>
              <p className={`text-2xl font-black ${
                item.color === 'purple' ? 'text-purple-900' :
                item.color === 'blue'   ? 'text-blue-900'   :
                item.color === 'green'  ? 'text-emerald-900' : 'text-amber-900'
              }`}>{loading ? '...' : item.value}</p>
              <p className={`text-sm font-bold mt-0.5 ${
                item.color === 'purple' ? 'text-purple-700' :
                item.color === 'blue'   ? 'text-blue-700'   :
                item.color === 'green'  ? 'text-emerald-700' : 'text-amber-700'
              }`}>{item.label}</p>
              <p className={`text-xs mt-0.5 opacity-70 ${
                item.color === 'purple' ? 'text-purple-600' :
                item.color === 'blue'   ? 'text-blue-600'   :
                item.color === 'green'  ? 'text-emerald-600' : 'text-amber-600'
              }`}>{item.sub}</p>
            </div>
          ))}
        </div>
