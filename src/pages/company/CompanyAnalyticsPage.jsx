  ...
  return (
    <div className={`bg-white border ${c.border} rounded-2xl p-5 shadow-sm flex flex-col gap-3`}>
      <div className="flex items-center gap-3">
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${c.bg}`}
        >
...
export default function CompanyAnalyticsPage() {
  ...
  const { data: interviews = [], isLoading: interviewsLoading } = useQuery({
    ...
    queryFn: () =>
      interviewService.list({ organization_id: orgId, sort: "date", order: "DESC", limit: 300 }),
    enabled: !!orgId,
    staleTime: STALE,
  })
  ...
  const stats = useMemo(() => {
    ...
    return {
      appsNow,
      appsPrev,
      pctApps: pctChange(appsNow, appsPrev),
      hiredNow,
      hiredPrev,
      pctHired: pctChange(hiredNow, hiredPrev),
      intNow,
      intPrev,
      pctInt: pctChange(intNow, intPrev),
      candNow,
      candPrev,
      pctCand: pctChange(candNow, candPrev),
      openJobs,
      convRate,
      funnelMap,
      srcMap,
            ...
            <table className="w-full text-sm" dir={isRTL ? "rtl" : "ltr"}>
              <thead>
                <tr className="border-b border-gray-100">
                  <th
                    className={`${isRTL ? "text-right" : "text-left"} text-xs text-gray-500 font-semibold pb-2`}
                  >
