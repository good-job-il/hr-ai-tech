            ...
            <>
              ...
              {interview.feedback ? (
                ...
                <p className="text-sm text-gray-400 italic">
                  {t("company.interviews.detail.noFeedback")}
                </p>
              )}
            </>
          )}
...
export default function CompanyInterviews({
  ...
  const {
    ...
    queryFn: () =>
      interviewService.list({ organization_id: orgId, sort: "date", order: "DESC", limit: 500 }),
    enabled: !!orgId,
    staleTime: 2 * 60 * 1000,
  })
  ...
  return (
    <div dir="rtl" className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <StatCard
          icon={Calendar}
          label={t("company.interviews.stats.total")}
          value={stats.total}
          color="#2563EB"
          loading={isLoading}
        />
      ...
      <div
        className={`grid gap-6 ${selected ? "grid-cols-1 lg:grid-cols-[1fr_400px]" : "grid-cols-1"}`}
      >
        {/* Interview cards */}
        <div>
          {isLoading ? (
            ...
            <div
              className={`grid gap-4 ${selected ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2 xl:grid-cols-3"}`}
            >
              {filtered.map((iv) => (
                <InterviewCard
                  key={iv.id}
