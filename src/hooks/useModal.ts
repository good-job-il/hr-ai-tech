...
export const useModal = (): UseModalReturn => {
  ...
  const openConfirmDialog = useCallback(async (config: ConfirmDialogConfig): Promise<boolean> => {
    return store.confirmDialog(config);
  }, [store]);

  const openAsyncDialog = useCallback(async (config: AsyncDialogConfig): Promise<any> => {
