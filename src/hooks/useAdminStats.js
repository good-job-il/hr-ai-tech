    ...
    case "custom":
      return {
        from: customFrom
          ...
          : startOfDay(new Date(now.setDate(now.getDate() - 29))),
        to: customTo ? endOfDay(new Date(customTo)) : endOfDay(new Date()),
      }
    default:
      return {
        from: startOfDay(new Date(now.setDate(now.getDate() - 6))),
...
export function useAdminStats(range, customFrom, customTo) {
  const { data: jobs = [], isLoading: l1 } = useQuery({
    ...
    queryFn: () =>
      httpClient.get("/jobs?sort=created_date&order=DESC&limit=1000", { cache: false }),
    staleTime: 60_000,
  })
  const { data: applications = [], isLoading: l2 } = useQuery({
    queryKey: ["admin-stats-apps"],
    queryFn: () =>
      httpClient.get("/applications?sort=created_date&order=DESC&limit=1000", { cache: false }),
    staleTime: 60_000,
  })
  const { data: candidates = [], isLoading: l3 } = useQuery({
    queryKey: ["admin-stats-candidates"],
    queryFn: () =>
      httpClient.get("/candidates/profiles?sort=created_date&order=DESC&limit=1000", {
        cache: false,
      }),
    staleTime: 60_000,
  })
  const { data: savedJobs = [], isLoading: l4 } = useQuery({
    ...
    queryFn: () =>
      httpClient.get("/jobs/saved?sort=created_date&order=DESC&limit=1000", { cache: false }),
    staleTime: 60_000,
  })
  ...
  const stats = useMemo(() => {
    ...
    const resumesUploadedPrev = applications.filter(
      (a) => a.resume_url && inPrev(a.created_date),
    ).length
    ...
    const employersInCur = new Set(
      jobs.filter((j) => j.employer_id && inCur(j.created_date)).map((j) => j.employer_id),
    ).size
    const employersInPrev = new Set(
      jobs.filter((j) => j.employer_id && inPrev(j.created_date)).map((j) => j.employer_id),
    ).size
    ...
    jobs.forEach((j) => {
      ...
      if (!employerStats[j.employer_id])
        employerStats[j.employer_id] = { id: j.employer_id, jobs: 0, apps: 0, views: 0 }
    ...
    return {
      jobsPublished,
      jobsPublishedPrev,
      pctJobsPublished: pct(jobsPublished, jobsPublishedPrev),
      jobsActive,
      jobsClosed,
      jobsClosedPrev,
      pctJobsClosed: pct(jobsClosed, jobsClosedPrev),
      jobsImported,
      jobsImportedPrev,
      pctJobsImported: pct(jobsImported, jobsImportedPrev),
      appsReceived,
      appsReceivedPrev,
      pctApps: pct(appsReceived, appsReceivedPrev),
      resumesUploaded,
      resumesUploadedPrev,
      pctResumes: pct(resumesUploaded, resumesUploadedPrev),
      newCandidates,
      newCandidatesPrev,
      pctCandidates: pct(newCandidates, newCandidatesPrev),
      employersInCur,
      employersInPrev,
      pctEmployers: pct(employersInCur, employersInPrev),
      savedCount,
      savedCountPrev,
      pctSaved: pct(savedCount, savedCountPrev),
      totalViews,
      convRate,
      chartDays,
      topJobs,
