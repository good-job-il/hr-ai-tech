    ...
    <footer dir={isRtl ? 'rtl' : 'ltr'} style={{ background: '#0F172A', color: 'white' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '56px 24px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr 1fr', gap: 40, marginBottom: 48 }}>
          {/* Brand */}
          <div>
            <img src={LOGO} alt="HeadHunter HR-Tech" style={{ height: 44, width: 'auto', objectFit: 'contain', marginBottom: 16 }} />
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, margin: '0 0 20px', maxWidth: 220 }}>
              {isRtl
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {[
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(124,58,237,0.3)'; e.currentTarget.style.color = 'white'; }}
          ...
          {Object.entries(LINKS).map(([category, links]) => (
            <div key={category}>
              <h4 style={{ fontSize: 13, fontWeight: 800, color: 'white', margin: '0 0 16px', letterSpacing: '0.02em' }}>{category}</h4>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)' }}>Made with ❤ in Israel</span>
          </div>
        </div>
      </div>
    </footer>
