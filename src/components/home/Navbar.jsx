      ...
      <div className="max-w-[1560px] mx-auto px-8 h-[86px] flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-7 text-[#172033] font-semibold">
          ...
          {user?.role === "admin" && (
            <Link
              to="/admin/dashboard"
              className="text-purple-600 flex items-center gap-1 hover:text-purple-700 transition"
            >
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="lg:hidden text-[#374151]" onClick={() => setMobileOpen(!mobileOpen)}>
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-white/95 backdrop-blur-md border-t border-[#E4ECFF] px-6 py-4 space-y-1">
          ...
          {user?.role === "admin" && (
            <Link
              to="/admin/dashboard"
              className="block text-purple-600 text-sm py-3 font-medium"
              onClick={() => setMobileOpen(false)}
            >
              🛡 {isRtl ? "ניהול" : "Admin"}
            </Link>
          ...
          <div className="flex gap-3 pt-4">
            {user ? (
              <>
                <Link
                  to={dashboardLink()}
                  className="flex-1 text-center py-3 rounded-xl border border-[#C9D8FF] text-[#6C4DFF] font-bold text-sm"
                  onClick={() => setMobileOpen(false)}
                >
                  {t("common.dashboard")}
                </Link>
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-gradient-to-l from-[#2F80FF] to-[#A855F7] text-white font-bold text-sm"
                >
                  {t("common.logout")}
                </button>
              </>
