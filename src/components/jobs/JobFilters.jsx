...
const CATEGORIES = ['פיתוח תוכנה', 'עיצוב', 'שיווק', 'מכירות', 'כספים', 'HR', 'הנדסה', 'רפואה', 'חינוך', 'לוגיסטיקה'];
...
export default function JobFilters({ isOpen, onClose, filters, onFilterChange, onSearch }) {
  ...
  return (
    <>
      ...
      <div className="fixed left-0 top-0 bottom-0 w-full sm:w-96 bg-gradient-to-b from-[#1a1f3a] to-[#0f1629] z-50 shadow-2xl overflow-y-auto border-l border-white/10" dir="rtl">
        <div className="sticky top-0 bg-gradient-to-b from-[#1a1f3a] to-[#0f1629] border-b border-white/10 px-6 py-5 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2.5 hover:bg-white/10 rounded-lg transition-colors active:scale-90 -mr-2">
        ...
        <div className="p-6 space-y-5">
          ...
          {filterSections.map(section => (
            <div key={section.id} className="border border-white/15 rounded-2xl bg-white/5 hover:border-white/25 transition-colors overflow-hidden">
              <button
              ...
              {expanded[section.id] && (
                <div className="border-t border-white/10 p-5 space-y-4 bg-white/5">
                  {section.options.map(opt => (
                    <label key={opt.value} className="flex items-center gap-3 cursor-pointer hover:bg-white/10 p-2.5 rounded-lg transition-all active:bg-white/15 -mx-2.5 px-3.5">
                      <input
                        type="checkbox"
