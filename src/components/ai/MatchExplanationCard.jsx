      ...
      <ul className="space-y-1">
        {items.map((item, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-[#374151]">
            <span
              className={`mt-0.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${color.replace("text-", "bg-")}`}
            />
            {item}
          </li>
        ))}
      ...
      {open && (
        <div className="p-5 space-y-5">
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-red-50 border border-red-200">
              ...
              <div>
                <div className="text-sm font-black text-red-700">
                  {t("aiMatching.explanationCard.missingRequirements")}
                </div>
                {explanation.missingRequired.map((m, i) => (
                  <div key={i} className="text-xs text-red-600 mt-0.5">
                    {m}
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <Section
              icon={CheckCircle2}
              title={t("aiMatching.explanationCard.strengths")}
              color="text-green-600"
              items={explanation.strengths}
            />
            <Section
              icon={XCircle}
              title={t("aiMatching.explanationCard.gaps")}
              color="text-red-500"
              items={explanation.gaps}
            />
            <Section
              icon={AlertTriangle}
              title={t("aiMatching.explanationCard.risks")}
              color="text-amber-500"
              items={explanation.risks}
            />
            <Section
              icon={Lightbulb}
              title={t("aiMatching.explanationCard.recommendations")}
              color="text-blue-500"
            ...
            <div className="border-t border-[#F1F5F9] pt-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-black text-[#0F172A]">
                  {t("aiMatching.explanationCard.screeningQuestions")}
                </span>
              </div>
          ...
          {explanation.nextAction && (
            <div className="flex items-center justify-between p-3 rounded-xl bg-[#F7FBFF] border border-[#E4ECFF]">
              <span className="text-sm text-[#64748B] font-semibold">
                {t("aiMatching.explanationCard.nextAction")}
              </span>
