...
export default function EmployerLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#F5FAFF] flex text-[#0F172A]" dir={dir}>
      <aside
        className={`fixed inset-y-0 z-40 w-[280px] bg-white/82 backdrop-blur-2xl flex flex-col transform transition-transform
        ${
          isEn
      ...
      >
        <Link
          to="/"
          className="h-[96px] px-6 border-b border-[#E4ECFF] flex items-center justify-center"
        >
            ...
            if (hasSubmenu) {
              return (
                <div key={item.label}>
                  ...
                  {isSubmenuOpen && (
                    <div
                      className={`mt-2 space-y-1 border-[#E4ECFF] ${isEn ? "ml-4 pl-3 border-l" : "mr-4 pr-3 border-r"}`}
                    >
        ...
        <div className="p-4 border-t border-[#E4ECFF]">
          ...
          <Link
            to="/"
            className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-[#64748B] hover:bg-[#F1F6FF]"
          >
      ...
      <div
        className={`flex-1 min-h-screen flex flex-col ${isEn ? "md:ml-[280px]" : "md:mr-[280px]"}`}
      >
        <header
          className="sticky top-0 z-30 h-16 md:h-[72px] lg:h-[88px] bg-white/70 backdrop-blur-2xl border-b border-white/60 shadow-sm px-4 md:px-6 lg:px-8 flex items-center justify-between"
          style={{ boxShadow: "0 1px 0 rgba(220,235,255,0.8), 0 4px 24px rgba(79,124,255,0.06)" }}
        >
          <div className="flex items-center gap-3">
            <button
              className="md:hidden w-9 h-9 rounded-full flex items-center justify-center hover:bg-white/80 transition"
          ...
          {/* Rounded-full search */}
          <div
            className="hidden lg:flex items-center gap-3 w-[380px] h-11 rounded-full bg-white/80 border border-white/70 px-5 shadow-sm backdrop-blur-xl"
            style={{
              boxShadow: "0 2px 12px rgba(79,124,255,0.07), inset 0 1px 0 rgba(255,255,255,0.9)",
            }}
          >
          ...
          <Link to="/" className="hidden md:flex">
            <img
              ...
              className="h-[48px] lg:h-[52px] w-auto object-contain"
            />
          </Link>
        </header>
