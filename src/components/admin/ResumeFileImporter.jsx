        ...
        <div className="px-4 pb-3 border-t border-[#F0F1F5] bg-[#F7F8FC] space-y-2 pt-3">
          ...
          {status.candidate_id && (
            <p className="text-xs text-[#94A3B8]">
              Candidate ID:{" "}
              <span className="font-mono font-bold text-[#7C3AED]">{status.candidate_id}</span>
              {status.document_id && (
                <>
                  {" "}
                  • Doc ID:{" "}
                  <span className="font-mono font-bold text-[#2563EB]">{status.document_id}</span>
                </>
              )}
            </p>
          )}
        ...
        >
          {running ? (
            <>
              ...
              מעבד {files.length} קבצים...
            </>
          ) : (
            <>
              <Upload className="w-4 h-4 ml-2" />
              ייבא {files.length || ""} קבצי קורות חיים
            </>
          )}
        </Button>
