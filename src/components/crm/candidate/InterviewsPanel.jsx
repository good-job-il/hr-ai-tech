...
export default function InterviewsPanel({ interviews, onSchedule, onUpdate }) {
  ...
  const STATUS_CFG = {
    scheduled:   { label: t('candidateCRM.interviews.statuses.scheduled'), color: 'bg-blue-100 text-blue-700' },
    confirmed:   { label: t('candidateCRM.interviews.statuses.confirmed'), color: 'bg-green-100 text-green-700' },
    completed:   { label: t('candidateCRM.interviews.statuses.completed'), color: 'bg-gray-100 text-gray-600' },
    cancelled:   { label: t('candidateCRM.interviews.statuses.cancelled'), color: 'bg-red-100 text-red-700' },
    no_show:     { label: t('candidateCRM.interviews.statuses.no_show'), color: 'bg-orange-100 text-orange-700' },
    rescheduled: { label: t('candidateCRM.interviews.statuses.rescheduled'), color: 'bg-yellow-100 text-yellow-700' },
  };
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-[#F7F8FC] rounded-xl p-4 mb-4 border border-[#E4ECFF]">
          <h4 className="font-black text-[#0F172A] mb-3 text-sm">{t('candidateCRM.interviews.scheduleNew')}</h4>
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <label className="text-xs font-bold text-[#64748B] mb-1 block">{t('candidateCRM.interviews.form.date')} *</label>
              <Input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className="text-sm" />
            </div>
            <div>
              <label className="text-xs font-bold text-[#64748B] mb-1 block">{t('candidateCRM.interviews.form.time')} *</label>
              <Input type="time" value={form.time} onChange={e => setForm(p => ({ ...p, time: e.target.value }))} className="text-sm" />
            </div>
            <div>
              <label className="text-xs font-bold text-[#64748B] mb-1 block">{t('candidateCRM.interviews.form.type')}</label>
            ...
            <div>
              <label className="text-xs font-bold text-[#64748B] mb-1 block">{t('candidateCRM.interviews.form.stage')}</label>
            ...
            <div className="col-span-2">
              <label className="text-xs font-bold text-[#64748B] mb-1 block">{t('candidateCRM.interviews.form.locationLink')}</label>
              <Input value={form.location_or_link} onChange={e => setForm(p => ({ ...p, location_or_link: e.target.value }))}
                placeholder={t('candidateCRM.interviews.form.locationPlaceholder')} className="text-sm" />
            </div>
            <div>
              <label className="text-xs font-bold text-[#64748B] mb-1 block">{t('candidateCRM.interviews.form.interviewer')}</label>
              <Input value={form.interviewer_name} onChange={e => setForm(p => ({ ...p, interviewer_name: e.target.value }))} className="text-sm" />
            </div>
            <div>
              <label className="text-xs font-bold text-[#64748B] mb-1 block">{t('candidateCRM.interviews.form.job')}</label>
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={handleSubmit} disabled={!form.date || !form.time || saving}
              ...
              {saving ? t('candidateCRM.interviews.saving') : t('candidateCRM.interviews.scheduleInterview')}
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setShowForm(false)} className="text-xs">{t('candidateCRM.interviews.cancel')}</Button>
          </div>
        </div>
      )}
      ...
      {upcoming.length > 0 && (
        <div className="mb-4">
          <div className="text-xs font-black text-[#94A3B8] uppercase tracking-wide mb-2">{t('candidateCRM.interviews.upcoming')}</div>
          <div className="space-y-2">
            {upcoming.map(i => <InterviewCard key={i.id} interview={i} onUpdate={onUpdate} typeLabels={TYPE_LABELS} stageLabels={STAGE_LABELS} statusCfg={STATUS_CFG} />)}
          </div>
        </div>
      )}
      ...
      {past.length > 0 && (
        <div>
          <div className="text-xs font-black text-[#94A3B8] uppercase tracking-wide mb-2">{t('candidateCRM.interviews.past')}</div>
          <div className="space-y-2">
            {past.map(i => <InterviewCard key={i.id} interview={i} onUpdate={onUpdate} typeLabels={TYPE_LABELS} stageLabels={STAGE_LABELS} statusCfg={STATUS_CFG} />)}
          </div>
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {interview.status === 'scheduled' && (
          <div className="flex gap-1">
            <Button size="sm" variant="outline" onClick={() => setShowFeedback(true)} className="text-xs h-7">{t('candidateCRM.interviews.feedback')}</Button>
            <Button size="sm" variant="outline" onClick={() => onUpdate?.(interview.id, { status: 'cancelled' })}
              className="text-xs h-7 text-red-500 hover:text-red-600">{t('candidateCRM.interviews.cancelInterview')}</Button>
          </div>
        )}
      </div>
      ...
      {showFeedback && (
        <div className="mt-3 border-t border-[#F0F1F5] pt-3">
          ...
          <div className="flex gap-2 mt-2">
            <Button size="sm" onClick={saveFeedback} className="bg-[#7C3AED] text-white text-xs">{t('candidateCRM.interviews.saveFeedback')}</Button>
            <Button size="sm" variant="ghost" onClick={() => setShowFeedback(false)} className="text-xs">{t('candidateCRM.interviews.cancel')}</Button>
          </div>
        </div>
      )}
    </div>
