...
const DOC_COLORS = {
  ...
  certificate: '#F59E0B', contract: '#EF4444', id: '#64748B', other: '#94A3B8',
};
...
export default function DocumentsPanel({ documents, candidate, onUpload }) {
  ...
  const hasResumeEntity = candidate?.original_resume_url || candidate?.converted_resume_url || candidate?.resume_url;
    ...
    <div className="flex items-center gap-3 bg-white rounded-xl border border-[#E4ECFF] px-4 py-3 group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {badge && <span className="text-xs bg-[#EEF4FF] text-[#4F46E5] font-bold px-2 py-0.5 rounded-full">{badge}</span>}
        </div>
        {doc.file_size && <span className="text-xs text-[#94A3B8]">{formatBytes(doc.file_size)}</span>}
      </div>
