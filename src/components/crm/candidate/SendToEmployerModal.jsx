  ...
  const allDocs = useMemo(() => {
    ...
    if (primaryCvUrl) {
      docs.push({
        ...
        url: primaryCvUrl,
        filename:
          candidate.original_resume_filename ||
          candidate.resume_filename ||
          t("candidateCRM.sendToEmployer.resumeFile"),
        doc_type: "cv",
    ...
    if (candidate?.converted_resume_url && candidate.converted_resume_url !== primaryCvUrl) {
      docs.push({
        ...
        filename:
          candidate.converted_resume_filename || t("candidateCRM.sendToEmployer.resumeDocx"),
        doc_type: "cv",
        badge: "DOCX",
      })
    }
    ...
    documents.forEach((doc) => {
      docs.push({
        ...
        filename:
          doc.filename || doc.original_filename || t("candidateCRM.sendToEmployer.document"),
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col overflow-hidden">
        ...
        {result && (
          <div
            className={`mx-6 mt-4 px-4 py-3 rounded-xl flex items-center gap-3 ${result.success ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
          >
            {result.success ? (
            ...
            >
              {sending ? (
                <>
                  ...
                  {t("candidateCRM.sendToEmployer.sending")}
                </>
              ) : result?.success ? (
                <>
                  <CheckCircle2 className="w-3.5 h-3.5" />{" "}
                  {t("candidateCRM.sendToEmployer.sendAgain")}
                </>
              ) : (
                <>
                  <Send className="w-3.5 h-3.5" /> {t("candidateCRM.sendToEmployer.sendEmail")}
                </>
              )}
            </Button>
          </div>
