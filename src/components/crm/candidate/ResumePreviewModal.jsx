      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E4ECFF]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-black text-[#0F172A]">{t('candidateCRM.resumeModal.title')} — {candidate.full_name}</h2>
          </div>
          <div className="flex items-center gap-2">
            {resumeUrl && (
              <>
          ...
          <div className="px-6 pt-4 flex flex-wrap gap-2">
            ...
            {originalUrl && (
              <a href={originalUrl} download
                ...
                <Download className="w-3.5 h-3.5" /> {t('candidateCRM.resumeModal.downloadOriginal', { type: candidate.original_file_type || t('candidateCRM.resumeModal.file') })}
              </a>
            )}
          </div>
        )}
            ...
            <div className="space-y-5">
              ...
              <Section title={t('candidateCRM.resumeModal.sections.details')}>
                <div className="grid grid-cols-2 gap-3">
                  {candidate.role_name && <InfoRow icon={<Briefcase className="w-4 h-4" />} label={t('candidateCRM.resumeModal.fields.role')} value={candidate.role_name} />}
                  {candidate.domain_name && <InfoRow icon={<Globe className="w-4 h-4" />} label={t('candidateCRM.resumeModal.fields.domain')} value={candidate.domain_name} />}
                  {candidate.experience_years && <InfoRow icon={<Briefcase className="w-4 h-4" />} label={t('candidateCRM.resumeModal.fields.experience')} value={t('candidateCRM.resumeModal.yearsValue', { count: candidate.experience_years })} />}
                  {candidate.location && <InfoRow icon={<MapPin className="w-4 h-4" />} label={t('candidateCRM.resumeModal.fields.location')} value={candidate.location} />}
                  {(candidate.desired_salary_min || candidate.desired_salary_max) && (
                    <InfoRow icon={<Briefcase className="w-4 h-4" />} label={t('candidateCRM.resumeModal.fields.salaryExpectations')}
