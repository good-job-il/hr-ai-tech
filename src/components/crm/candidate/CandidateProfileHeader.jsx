      ...
      <div className="flex flex-col md:flex-row gap-4 items-start">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            <span className={`text-xs font-bold px-3 py-1 rounded-full ${STATUS_COLORS[candidate.status] || 'bg-gray-100 text-gray-600'}`}>
          ...
          <div className="flex flex-wrap gap-3 text-sm text-[#64748B]">
            ...
            {candidate.email && (
              <a href={`mailto:${candidate.email}`} className="flex items-center gap-1 hover:text-[#7C3AED] transition-colors">
                <Mail className="w-3.5 h-3.5" />{candidate.email}
              </a>
            )}
            {candidate.phone && (
              <a href={`tel:${candidate.phone}`} className="flex items-center gap-1 hover:text-[#7C3AED] transition-colors">
            ...
            {candidate.desired_salary_min && (
              <span className="flex items-center gap-1">
                ...
                {candidate.desired_salary_max && `–${candidate.desired_salary_max?.toLocaleString()}`}
              </span>
            )}
          </div>
          ...
          {tags?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {tags.map(t => (
                <span key={t.id} className="text-xs font-semibold px-2.5 py-1 rounded-full border"
                  style={{ borderColor: t.color || '#7C3AED', color: t.color || '#7C3AED', background: `${t.color || '#7C3AED'}15` }}>
                  {t.tag}
                </span>
              ))}
            </div>
