...
export default function WhatsAppPanel({ candidate, communications, onAddCommunication }) {
  ...
  return (
    <div className="space-y-4" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Templates */}
      <div>
        <div className="text-xs font-black text-[#94A3B8] uppercase tracking-wide mb-2">{t('candidateCRM.whatsapp.messageTemplates')}</div>
      ...
      {/* Message editor */}
      <div>
        <div className="text-xs font-black text-[#94A3B8] uppercase tracking-wide mb-2">{t('candidateCRM.whatsapp.message')}</div>
        <textarea
          value={customMsg}
      ...
      {showLog ? (
        <div className="bg-[#F0FDF4] border border-green-200 rounded-xl p-4 space-y-3">
          <div className="text-xs font-black text-[#15803D]">{t('candidateCRM.whatsapp.documentConversation')}</div>
          <textarea
            value={logSummary}
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={() => setShowLog(false)} className="text-xs">{t('candidateCRM.whatsapp.cancel')}</Button>
          </div>
        </div>
      ) : (
      ...
      {waCommunications.length > 0 && (
        <div>
          <div className="text-xs font-black text-[#94A3B8] uppercase tracking-wide mb-2">{t('candidateCRM.whatsapp.history')}</div>
              ...
              <div key={c.id || i} className="flex items-start gap-3 px-3 py-2.5 bg-white rounded-xl border border-[#E4ECFF]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    <span className="text-xs text-[#94A3B8]">{new Date(c.created_date).toLocaleDateString(currentLang === 'he' ? 'he-IL' : 'en-US')}</span>
