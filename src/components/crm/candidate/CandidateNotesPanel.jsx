...
export default function CandidateNotesPanel({ notes, onAddNote, onUpdateNote, onDeleteNote, userRole }) {
  ...
  const VISIBILITY_CONFIG = {
    internal:         { icon: Lock,    label: t('candidateCRM.notesPanel.visibility.internal'), color: 'text-[#64748B]', bg: 'bg-[#F0F1F5]' },
    employer_visible: { icon: Eye,     label: t('candidateCRM.notesPanel.visibility.employer_visible'), color: 'text-green-700', bg: 'bg-green-50' },
    all:              { icon: Eye,     label: t('candidateCRM.notesPanel.visibility.all'), color: 'text-blue-700', bg: 'bg-blue-50' },
  };
  ...
  return (
    <div>
        ...
        <div className="bg-[#F7F8FC] rounded-xl p-4 mb-4 border border-[#E4ECFF]">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            <button onClick={() => setIsPinned(!isPinned)}
              ...
              <Pin className="w-3 h-3" /> {isPinned ? t('candidateCRM.notesPanel.pinned') : t('candidateCRM.notesPanel.pin')}
            </button>
          </div>
      ...
      {pinned.length > 0 && (
        <div className="mb-4">
          ...
          <div className="space-y-2">
            {pinned.map(note => <NoteCard key={note.id} note={note} onDelete={onDeleteNote} onUpdate={onUpdateNote} visibilityConfig={VISIBILITY_CONFIG} typeLabels={TYPE_LABELS} dateLocale={dateLocale} />)}
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        {regular.map(note => <NoteCard key={note.id} note={note} onDelete={onDeleteNote} onUpdate={onUpdateNote} visibilityConfig={VISIBILITY_CONFIG} typeLabels={TYPE_LABELS} dateLocale={dateLocale} />)}
      </div>
    </div>
  );
}
