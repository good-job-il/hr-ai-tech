...
export default function DashboardHeader({
  ...
  if (platformStyle) {
    ...
    const displayName =
      user?.full_name?.trim() ||
      user?.email?.split("@")[0] ||
      (isRtl ? "מנהל מערכת" : "Platform Admin")

    return (
      <header
        className="relative z-30 grid h-[82px] shrink-0 grid-cols-[auto_minmax(280px,470px)_1fr] items-center gap-6 border-b border-[#E9EDF6] bg-white px-5 shadow-[0_2px_12px_rgba(54,74,138,0.035)] max-lg:grid-cols-[auto_1fr] max-md:h-[70px] max-md:px-4"
        dir="ltr"
      >
        <div className="flex min-w-max items-center gap-3">
          ...
          <button
            className="relative flex h-11 w-11 items-center justify-center rounded-xl text-[#66708B] transition hover:bg-[#F6F4FF]"
          ...
          <button
            className="hidden h-11 items-center gap-3 rounded-[14px] border border-[#E6E9F1] bg-white px-4 text-sm font-bold text-[#5E6881] shadow-[0_3px_12px_rgba(54,74,138,0.035)] transition hover:border-[#D9D2FF] hover:bg-[#FAF9FF] lg:flex"
            dir={isRtl ? "rtl" : "ltr"}
          >
