...
export default function EditNameModal({ user, onUpdated }) {
  ...
  if (!open) {
    return (
      <button
        onClick={() => { setName(user?.full_name || ''); setOpen(true); }}
  ...
  return (
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-2xl shadow-2xl border border-[#E4ECFF] p-6 w-80" dir="rtl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-black text-[#0F172A]">עדכון שם מלא</h3>
          <button onClick={() => setOpen(false)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-[#F0F1F5]">
