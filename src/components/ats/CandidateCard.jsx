...
export default function CandidateCard({ application, stageColor, slaHours, onClick, isDragging }) {
  ...
  const slaBreached = slaHours && application.stage_entered_at &&
    ...
    <div
      ...
      style={{
        boxShadow: isDragging
          ? '0 20px 60px rgba(108,77,255,0.25)'
      ...
      {application.tags && application.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          {application.tags.slice(0, 3).map(tag => (
            <span key={tag} className="text-xs bg-[#F0F4FF] text-[#2F80FF] px-2 py-0.5 rounded-full font-semibold">
              {tag}
            </span>
          ))}
        </div>
