    ...
    <header className={cn('flex flex-col justify-between gap-4 sm:flex-row sm:items-center', className)}>
      <div>
        <div className="flex items-center gap-2">
          <h1 className="text-[26px] font-black tracking-tight text-slate-900 sm:text-[30px]">{title}</h1>
...
export function PlatformStatCard({
  ...
  const content = (
    ...
    >
      <div className={cn('absolute -right-8 -top-9 h-24 w-24 rounded-full opacity-25 blur-2xl', colors.glow)} />
        ...
        <div>
          ...
          {loading ? (
            ...
            <p className="mt-2 text-[28px] font-black tracking-tight text-slate-900">
              {prefix}{formatPlatformNumber(value)}{suffix}
            </p>
          )}
        </div>
        {Icon && (
          <div className={cn('flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br', colors.iconBg)}>
      ...
      {(change !== undefined || meta || to) && (
        <div className="relative mt-3 flex items-center gap-1 text-[11px] font-semibold">
          ...
          {to && <ArrowUpRight className="ml-auto h-3.5 w-3.5 text-slate-300 transition group-hover:text-violet-500" />}
        </div>
      )}
    </div>
  );
...
export function PlatformModal({
  ...
  const colors = statTones[tone] || statTones.violet;
  return (
    <div dir={dir} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm">
      <div className={cn('w-full rounded-[24px] border border-white/80 bg-white p-6 shadow-2xl', maxWidth)}>
        <div className="mb-5 flex items-start gap-3">
          {Icon && (
            <div className={cn('flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br', colors.iconBg, colors.icon)}>
              <Icon className="h-5 w-5" />
            </div>
          )}
...
export const platformFieldClassName = 'w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-300 focus:border-violet-400 focus:ring-4 focus:ring-violet-50';
