...
export const managementReportService = {
  get: (query: ManagementReportQuery) => httpClient.get<ManagementReport>(`/management-reports${buildQuery(query)}`, { cache: false }),
  export: (query: ManagementReportQuery) => httpClient.get<ManagementReport>(`/management-reports/export${buildQuery(query)}`, { cache: false }),
};
