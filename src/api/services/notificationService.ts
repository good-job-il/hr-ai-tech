...
export interface NotificationRecord {
  id: number;
  type: 'new_application' | 'interview_scheduled' | 'message' | 'job_closed' | 'job_match' | 'interview_reminder';
  title: string;
  content: string | null;
...
export const notificationService = {
  async list(query: { is_read?: boolean; type?: NotificationType; page?: number; limit?: number } = {}): Promise<NotificationRecord[]> {
    const response = await httpClient.get<NotificationRecord[] | PaginatedResponse<NotificationRecord>>(
