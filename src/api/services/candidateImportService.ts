...
class CandidateImportService extends ResourceService<ImportBatchRecord, ResourceQuery, CreateImportBatchInput, never> {
  constructor() { super('/candidates/import-batches'); }

  queueFileImport(batchId: number, fileUrl: string, fileName?: string) {
    return httpClient.post<BackgroundJob<CandidateImportResult>>(`/candidate-imports/batches/${batchId}/run`, {
      file_url: fileUrl,
      file_name: fileName,
  ...
  retryBatch(batchId: number) {
    return httpClient.post<BackgroundJob<CandidateImportResult>>(`/candidate-imports/batches/${batchId}/retry`);
  }
  ...
  parseBatch(payload: ParseResumeBatchInput) {
    return httpClient.post<Record<string, unknown>>('/candidate-imports/resume-batches/parse', payload);
  }

  validateBatch(importBatchId: number) {
    return httpClient.post<Record<string, unknown>>('/candidate-imports/validate', { import_batch_id: importBatchId });
  }
