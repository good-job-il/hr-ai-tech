...
class ImportSourceService extends ResourceService<ImportSourceRecord, ResourceQuery, ImportSourceInput, Partial<ImportSourceInput>> {
  ...
  preview(url: string, companyName: string) {
    return httpClient.post<ImportRunResult>('/import-sources/preview', { url, company_name: companyName });
  }
}
