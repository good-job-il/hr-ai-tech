...
function mutableCollection<TEntity, TQuery extends object, TCreate, TUpdate>(
  endpoint: `/${string}`,
) {
  return {
    ...
    update: (id: number, payload: TUpdate) =>
      httpClient.patch<TEntity>(`${endpoint}/${id}`, payload),
...
export const effectivePermissionService = {
  get: () =>
    httpClient.get<EffectivePermissionsResponse>("/permissions/effective", { cache: false }),
}
