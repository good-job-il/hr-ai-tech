...
export const communicationService = {
  async list(query: { candidate_id?: number; channel?: CommunicationLogInput['channel']; limit?: number } = {}) {
  ...
  presentCandidate: (candidate_id: number, job_id: number, recruiter_note?: string) =>
    httpClient.post<{ success: true; communication_log_id: number }>('/communication-logs/present-candidate', {
      candidate_id, job_id, recruiter_note,
    }),
};
