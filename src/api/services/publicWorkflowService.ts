...
export const publicWorkflowService = {
  ...
  recommendedJobs: () =>
    httpClient.get<{ jobs: Array<Job & { match_score: number }> }>('/matching/jobs/recommended', { cache: false }),
  extractResume: (fileUrl: string) =>
    httpClient.post<{ success: boolean; data: ExtractedResumeData }>('/resumes/extract', { file_url: fileUrl }),
