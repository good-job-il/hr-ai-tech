...
useEffect(() => { if (user) loadCandidates(); }, [statusFilter, user?.email, location.key]);
...
>
  {({ index, style }) => (
    <CandidateRowMemo style={style} candidate={filtered[index]} />
  )}
</FixedSizeList>
```
...
<Link
  to="/agency/crm"
  onMouseEnter={() => queryClient.prefetchQuery(['agency-candidates', orgId])}
>
  CRM
</Link>
```
