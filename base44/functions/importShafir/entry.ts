...
const EMPLOYER_ID = 'shafir_import';
...
Deno.serve(async (req) => {
  try {
    ...
    if (!feedUrl) return Response.json({ error: 'URL לא הוגדר' }, { status: 400 });
    ...
    const parseResult = await base44.asServiceRole.integrations.Core.InvokeLLM({
      prompt: `Extract all job listings from this content (JSON, XML, or HTML). For each job extract:
- title: job title in Hebrew
- location: city in Hebrew
- category: job category in Hebrew
- type: one of "full","part","remote","daily"
- description: short description, remove any company name/email/phone/contact info
- external_id: any unique ID from the source
